/**
 * Family Zoo Tutorial — Version 18: Sights & Sounds (Presentation)
 *
 * NEW IN THIS VERSION (all in presentation.ts + the wiring below):
 *   - A custom `zoo.ambience` text channel — a per-area mood line
 *   - Room soundscapes — ambient loops that swap as you move (media.ambient.*)
 *   - Background images per area (media.image.show on the image:background channel)
 *   - Sound effects — feed crunch, camera shutter (media.sound.play)
 *   - After-hours music — fades in when the zoo closes (media.music.play)
 *   - A story-shipped theme (zoo-sunny) — see browser/familyzoo.css
 *   Presentation rides the channel surface end to end; the engine special-cases none of it.
 *
 * INHERITED FROM V17:
...
// --- Our files ---
import { createZooMap, RoomIds } from './zoo-map.js';
import { createZooItems, ItemIds } from './zoo-items.js';
import {
  createCharacters, CharacterIds, PettableTrait,
  parrotBehavior, parrotAfterHoursBehavior, KEEPER_PATROL_ID,
} from './characters.js';
import { MAX_SCORE, ScoreIds, ScorePoints, ROOM_SCORE_MAP } from './scoring.js';
import {
  createPAAnnouncementDaemon, createFeedingTimeFuse,
  createGoatBleatingDaemon, createVictoryDaemon, createAfterHoursDaemons,
} from './events.js';
import { registerMessages, FeedMessages, PhotoMessages, PetMessages } from './language.js';
import {
  registerZooPresentationChannels, initializeZooAudio, registerZooAudioHandler,
  crunchSfx, shutterSfx, afterHoursMusic,
...
const feedAction: Action = {
  ...
  report(context: ActionContext): ISemanticEvent[] {
    ...
    // V18: the feed dispenser makes a sound — emit a media.sound.play event.
    return [
      context.event('zoo.event.fed', { messageId, params: { animal: target?.get(IdentityTrait)?.name } }),
      crunchSfx(),
    ];
  },
...
const photographAction: Action = {
  ...
  report(context: ActionContext): ISemanticEvent[] {
    ...
    // V18: the camera clicks — emit a media.sound.play event alongside the prose.
    return [
      context.event('zoo.event.photographed', { messageId: PhotoMessages.TOOK_PHOTO, params: { target: name } }),
      shutterSfx(),
    ];
  },
  blocked(context: ActionContext, result: ValidationResult): ISemanticEvent[] {
...
class FamilyZooStory implements Story {
  ...
  initializeWorld(world: WorldModel): void {
    ...
    // Register room soundscapes & background images (NEW IN V18)
    initializeZooAudio({
      entrance: rooms.entrance,
      pettingZoo: rooms.pettingZoo,
      aviary: rooms.aviary,
      nocturnalExhibit: rooms.nocturnalExhibit,
  ...
  // Register the story's presentation channels (NEW IN V18): the custom
  // `zoo.ambience` text channel and the `ambient:environment` soundscape
  // channel. The standard `image:background`, `sound`, and `music`
  // channels are platform defaults — no registration needed.
  registerChannels(registry: import('@sharpee/if-domain').IChannelRegistry): void {
    registerZooPresentationChannels(registry);
  }
  ...
  onEngineReady(engine: GameEngine): void {
    ...
    // --- Soundscape & background images (NEW IN V18) ---
    // Emits media.ambient.* and media.image.* on room entry.
    registerZooAudioHandler(engine.getEventProcessor());
    ...
    scheduler.registerDaemon({
      ...
      run: () => {
        ...
        // V18: the zoo has closed — fade in the after-hours theme. The
        // after-hours animal daemon still handles the dialogue.
        return [afterHoursMusic()];
      },
