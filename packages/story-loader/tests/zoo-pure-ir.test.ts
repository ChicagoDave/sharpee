 ...
 * gate): the pure-IR profile must refuse the shipped Zoo (it declares 3
 * `define text ... from` hatches) without reading a single hatch export,
...
describe('AC-4 sweep: the shipped zoo.story', () => {
  ...
  it('a hatch-stripped variant loads and builds a world under pure-IR', () => {
    // Strip the three `define text ... from` declarations and the prose
    // markers that reference them — everything else is the real Zoo.
    const stripped = source
      .split('\n')
      .filter((line) => !/^define text \S+ from/.test(line))
      .map((line) => line.replace(/\{(flavor|aside|gate-status)\}/g, ''))
      .join('\n');
    ...
    expect(world.getMaxScore()).toBe(100);
  });
});
