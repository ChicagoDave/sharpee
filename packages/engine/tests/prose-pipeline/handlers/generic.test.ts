/**
 * Tests for `handleGameMessage` and `handleGenericEvent` —
 * ported from text-service.
 *
...
import {
  handleGameMessage,
  handleGenericEvent,
} from '../../../src/prose-pipeline/handlers/generic';
...
describe('handleGameMessage', () => {
  it('should resolve messageId through language provider', () => {
    ...
    const event = makeEvent('game.message', { messageId: 'game.welcome' });
  ...
  it('should fall back to message field', () => {
    const event = makeEvent('game.message', { message: 'Direct message.' });
...
describe('handleGenericEvent', () => {
  ...
  it('should use direct text field when message is absent', () => {
    const event = makeEvent('story.custom', { text: 'Custom text.' });
  ...
  it('should resolve event type as template key through provider', () => {
    ...
    const event = makeEvent('if.event.custom_thing', { target: 'the lamp' });

    const blocks = handleGenericEvent(event, makeContext(provider));

    expect(blocks).toHaveLength(1);
    expect(blocks[0].content).toEqual([
      'The custom thing happened to the lamp!',
    ]);
  });
  ...
  it('should return empty when nothing resolves', () => {
    const provider = makeProvider({});
    const event = makeEvent('unknown.type', { someData: 'irrelevant' });
