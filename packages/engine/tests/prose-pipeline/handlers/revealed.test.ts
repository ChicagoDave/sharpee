/**
 * Tests for `handleRevealed` — ported from text-service.
 *
...
import { handleRevealed } from '../../../src/prose-pipeline/handlers/revealed';
...
describe('handleRevealed', () => {
  it('should use direct message field when present', () => {
    ...
    expect(blocks[0].content).toEqual([
      'Inside the chest you see a gleaming sword.',
    ]);
  });

  it('should use direct text field when message is absent', () => {
    const event = makeEvent('if.event.revealed', { text: 'You find a key.' });
  ...
  it('should return empty when no message, no provider match, and no items', () => {
    const event = makeEvent('if.event.revealed', { containerName: 'empty box' });
