 ...
 */
import { TokenType, POSType, PhraseType } from './types';
/**
 ...
 */
export class BaseIFParserPlugin {
    /**
     ...
     */
    extractVerbPhrase(tagged, start) {
        const words = [tagged[start]];
        let end = start;
     ...
     */
    extractNounPhrase(tagged, start) {
        const words = [];
        let end = start;
        let hasNoun = false;
     ...
     */
    extractPrepositionalPhrase(tagged, start) {
        const words = [tagged[start]]; // The preposition
        let end = start;
    ...
    analyzeGrammar(tagged) {
        ...
        const directObjectPhrase = phrases.find(p => p.type === PhraseType.NOUN_PHRASE &&
     ...
     */
    extractNounAndAdjectives(phrase) {
        const adjectives = [];
        let noun = '';
    ...
    extractNounFromPhrase(phrase) {
        const nouns = phrase.words.filter(w => w.tag === POSType.NOUN || w.tag === POSType.PRONOUN);
        if (nouns.length === 0) {
            return null;
        }
        // Join compound nouns
