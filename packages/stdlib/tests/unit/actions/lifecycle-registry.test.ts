...
describe('wired-action registry (ADR-228 D5)', () => {
  test('covers all 40 entity-keyed standard actions with unique primary ids', () => {
    ...
    expect(actionLifecycleDescriptors).toHaveLength(40);
    const primaryIds = actionLifecycleDescriptors.map((d) => d.actionId);
    expect(new Set(primaryIds).size).toBe(40);
