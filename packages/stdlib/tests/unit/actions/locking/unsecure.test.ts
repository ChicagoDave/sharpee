...
import { unlockingAction } from '../../../../src/actions/standard/locking';
...
describe('unlockingAction (Golden Pattern)', () => {
  ...
  describe('Key Requirements', () => {
    test('should fail when key required but not provided', () => {
      const { world, player, room } = TestData.withObject('oak door', {
        [TraitType.LOCKABLE]: {
          type: TraitType.LOCKABLE,
          isLocked: true,
          keyId: 'brass_key'  // Requires key (using keyId property)
        }
      });
    ...
    test('should fail when key not held by player', () => {
      ...
      // Add lockable trait with the key requirement
      safe.add({
        type: TraitType.LOCKABLE,
        isLocked: true,
        keyId: key.id
    ...
    test('should fail with wrong key', () => {
      ...
      const correctKey = world.createEntity('cabinet key', 'object'); // Create the correct key
      const wrongKey = world.createEntity('desk key', 'object');
      
      // Add lockable trait that requires the cabinet key
      cabinet.add({
        type: TraitType.LOCKABLE,
        isLocked: true,
        keyId: correctKey.id  // Requires cabinet key, not desk key
      });
  ...
  describe('Successful Unlocking', () => {
    test('should unlock object without key requirement', () => {
      ...
      expectEvent(events, 'if.event.unsecured', {
        target: latch.id
      });
    ...
    test.skip('should unlock with correct key', () => {
      ...
      chest.add({
        type: TraitType.LOCKABLE,
        isLocked: true,
        key: key.id  // Use actual entity ID
      });
  ...
  describe('Auto-Open Behavior', () => {
    ...
    test.skip('should not auto-open if not configured', () => {
      ...
      const unlockedEvent = events.find(e => e.type === 'if.event.unsecured');
