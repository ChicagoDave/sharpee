/**
 * Tests for handleAudibilityHeard handler — ADR-172 Phase 7a.
 *
 * Verifies that `sound.audibility.heard` events from the engine's sound
 * dispatcher are converted into prose blocks via the
 * `sound.heard.<kind>.<tier>` template family, with default-template
...
import { handleAudibilityHeard } from '../../src/handlers/audibility.js';
...
describe('handleAudibilityHeard', () => {
  ...
  it('falls back to default template when kind-specific template is missing', () => {
    const provider = makeProvider({
      // no 'sound.heard.thunderclap.presence-only' — must fall back
  ...
  it('produces one block per event in the order called', () => {
    ...
    const events = [
      makeEvent('sound.audibility.heard', {
        sourceRoomId: 'r1', targetRoomId: 'r1', sourceEntityId: 'e1',
        kind: 'ambient', volumeTier: 'normal', audibilityTier: 'full',
        timestamp: 4,
      }),
      makeEvent('sound.audibility.heard', {
        sourceRoomId: 'r2', targetRoomId: 'r1', sourceEntityId: 'e2',
        kind: 'ambient', volumeTier: 'subdued', audibilityTier: 'fragments',
        timestamp: 4,
      }),
    ];
    ...
    expect(blocks[1].content).toEqual(['You catch the faint sound of ambient.']);
  });
  ...
  it('returns empty array for silent tier (defense-in-depth)', () => {
    ...
    const event = makeEvent('sound.audibility.heard', {
      sourceRoomId: 'r1', targetRoomId: 'r1', sourceEntityId: 'e1',
      kind: 'speech', volumeTier: 'whisper', audibilityTier: 'silent',
      timestamp: 5,
    });
  ...
  it('returns empty array when languageProvider is absent', () => {
    const event = makeEvent('sound.audibility.heard', {
      sourceRoomId: 'r1', targetRoomId: 'r1', sourceEntityId: 'e1',
      kind: 'speech', volumeTier: 'normal', audibilityTier: 'full',
