/**
 * message-alias-catalog.ts — GENERATED by `repokit aliases` from
 * packages/story-loader/src/message-alias-map.ts. DO NOT EDIT — add the row to
 * the loader's curated map and regenerate; the freshness gate
 * (`repokit aliases --check`, run by `repokit verify`) fails the build on drift.
 *
 * The closed set of kebab aliases an author may name in
 * `override message <alias>`: the keys of the loader's curated alias ->
 * message-id table, names only. The analyzer checks `override message`
 * against this set and reports an unknown alias with a suggestion. The
 * dotted platform ids stay on the loader side, so the compiler stays
 * platform-free; the loader's completeness test pins its table to the
 * live message ids and this set to its keys.
 *
 * Public interface: MESSAGE_OVERRIDE_ALIASES.
 * Owner context: @sharpee/chord (language frontend; browser-safe; generated artifact).
 *
 * 742 aliases across 58 actions.
 *
 * References:
 * - ADR-255 D2 — one hand-maintained table; Interface Contract 3 — names
 *   only on the Chord side.
 * - ADR-335 D4 — this set is derived from the loader's table.
 */
