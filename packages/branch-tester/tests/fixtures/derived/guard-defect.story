...
## whose taking removes it and wins — plus a planted defect (AC-3): an
## every-turn clause that fruits the vine whatever the guard said, so the
## refusal's negative space fails on state, not on the message. Not story
## content.

story
  title: Derived Guard Defect
  authors:
    Sharpee
  id: derived-guard-defect
  story-version: 0.0.1
  states: calm, alarmed
  use scoring
