...
describe('sharpee test --bless (ADR-294 D1, issue #239)', () => {
  it('records a golden for a flat transcript, and the recording replays green', async () => {
    const gold = join(projectDir, 'tests', 'transcripts', 'd-gold.transcript');
    const recording = join(projectDir, 'tests', 'transcripts', 'd-gold.golden');
    // Golden shape: bare commands, a pinned seed (D3). Without a recording
    // this very file is the ADR-294 D2 error that tells an author to run
    // --bless — the flag the command used to answer with `unknown flag`.
    writeFileSync(gold, 'title: Gold\nseed: 42\n---\n\n> look\n\n> examine the brass lamp\n');
    try {
      const blessed = await run(['--json', projectDir, gold, '--bless']);
      expect(blessed.code).toBe(0);
      expect(ofType<TranscriptEndEvent>(blessed.records, 'transcript-end')[0].status).toBe('passed');

      const replayed = await run(['--json', projectDir, gold]);
      expect(replayed.code).toBe(0);
      expect(ofType<TranscriptEndEvent>(replayed.records, 'transcript-end')[0].status).toBe('passed');
    } finally {
      rmSync(gold, { force: true });
      rmSync(recording, { force: true });
    }
  });

  it('a chain member after the first records at the session seed — the latent chain gap', async () => {
    // The runner refused chain-member goldens with "replay it with --chain"
    // while --chain was exactly what was running, because the command never
    // passed `chain` through. Pinned here on the real path.
    const chainDir = mkdtempSync(join(tmpdir(), 'devkit-chain-bless-'));
    try {
      writeFileSync(join(chainDir, 'mini.story'), STORY);
      mkdirSync(join(chainDir, 'walkthroughs'), { recursive: true });
      writeFileSync(
        join(chainDir, 'walkthroughs', 'wt-01-take.transcript'),
        'title: Step 1\nseed: 42\n---\n\n> take the brass lamp\n',
      );
      writeFileSync(
        join(chainDir, 'walkthroughs', 'wt-02-carry.transcript'),
        'title: Step 2\n---\n\n> inventory\n',
      );

      const blessed = await run(['--json', '--chain', chainDir, '--bless']);
      expect(blessed.code).toBe(0);
      expect(
        ofType<TranscriptEndEvent>(blessed.records, 'transcript-end').map((e) => e.status),
      ).toEqual(['passed', 'passed']);

      const replayed = await run(['--json', '--chain', chainDir]);
      expect(replayed.code).toBe(0);
      expect(
        ofType<TranscriptEndEvent>(replayed.records, 'transcript-end').map((e) => e.status),
      ).toEqual(['passed', 'passed']);
    } finally {
  ...
  it('--bless-file without --tree is refused with guidance', async () => {
    const { code, err } = await run(['--json', projectDir, '--bless-file', 'x.transcript']);
    expect(code).toBe(2);
    expect(err).toContain('--bless-file requires --tree');
  });

  it('--tree --bless-file records ONLY the named child, at the root session seed', async () => {
    const treeBless = mkdtempSync(join(tmpdir(), 'devkit-tree-bless-'));
    try {
      writeFileSync(join(treeBless, 'mini.story'), STORY);
      mkdirSync(join(treeBless, 'tests', 'transcripts'), { recursive: true });
      const spine = join(treeBless, 'tests', 'transcripts', 'spine.transcript');
      const lamp = join(treeBless, 'tests', 'transcripts', 'lamp.transcript');
      writeFileSync(spine, 'title: Spine\nseed: 42\n---\n\n> look\n[OK: contains "A small square den"]\n');
      // The child is golden-shaped and pins nothing: its seed is its root's,
      // which is the case the resolvedConfig seam exists for.
      writeFileSync(lamp, 'title: Lamp\ncontinues: spine\n---\n\n> examine the brass lamp\n');

      const blessed = await run(['--json', '--tree', treeBless, '--bless-file', lamp]);
      expect(blessed.code).toBe(0);
      const { existsSync } = await import('node:fs');
      expect(existsSync(join(treeBless, 'tests', 'transcripts', 'lamp.golden'))).toBe(true);
      expect(existsSync(join(treeBless, 'tests', 'transcripts', 'spine.golden'))).toBe(false);

      // And the tree replays it: the child runs golden-tier, green, at the
      // root's pin — while the untouched root stays assertion-tier.
      const replayed = await run(['--json', '--tree', treeBless]);
      expect(replayed.code).toBe(0);
      expect(
        ofType<TranscriptEndEvent>(replayed.records, 'transcript-end').map((e) => e.status),
      ).toEqual(['passed', 'passed']);
    } finally {
      rmSync(treeBless, { recursive: true, force: true });
    }
  });
});
