...
By default, players navigate with compass directions (NORTH, SOUTH, EAST, WEST). Stories can replace these words with alternative vocabularies — for example, a ship uses FORE, AFT, PORT, and STARBOARD.

### Quick Setup

One line in `initializeWorld`:

```typescript
initializeWorld(world: WorldModel): void {
  world.directions().useVocabulary('naval');
  // ... create rooms as usual
}
```

After this call, the parser accepts `fore`, `f`, `forward` instead of `north`, `n`. Exit listings and messages display `fore` instead of `north`. Room connections still use `Direction.NORTH` internally — the vocabulary only affects what the player types and what the game displays.

### Built-In Vocabularies

| Vocabulary | Directions Available |
|---|---|
| `'compass'` (default) | north, south, east, west, ne, nw, se, sw, up, down, in, out |
| `'naval'` | fore, aft, port, starboard, topside, below decks, in, out |
| `'minimal'` | up, down, in, out |

### Custom Overrides

Rename individual directions:
...
Or add aliases without replacing the originals:

```typescript
world.directions()
  .alias(Direction.UP, { display: 'topside', words: ['topside', 'ts'] });
// Both 'up' and 'topside' work; displays as 'topside'
```

See the [Direction Vocabularies guide](/docs/author-guide/directions/) for full details.
...
6. **Choose the right vocabulary**: Naval for ships, minimal for caves, custom for fantasy worlds
