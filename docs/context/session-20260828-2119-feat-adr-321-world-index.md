...
## Phase 6a/6b (after the Phase 5 commit, 22:20–23:05 CDT)
- Dungeo rewritten onto the pipeline: troll/robot/cyclops/dungeon-master/thief behaviors use `context.act`/`narrate` (void hooks; the never-called `onSpokenTo`/`onAttacked` dropped — Dungeo's say/set-dial/push-dial-button/answer actions own those mechanics). `MeleeInterceptor` gained the villain→hero branch (the deleted `melee-npc-attack.ts` resolver's logic; `VILLAIN_RECOVERS`/`VILLAIN_NO_STRENGTH` refusals; hero death via `killPlayer` inside the interceptor, emitted from `postReport`, so attacking's `combat.player_died` line is not added). `melee-villain-blow.test.ts` (4) replaces the resolver test. `orchestration/index.ts` uses `engine.getNpcService()`. `troll-axe-behaviors.ts` exempts the guardian from the white-hot refusal (the troll recovers his own axe through the real `taking`).
- New Dungeo action `actions/stealing/` (`DUNGEO_STEAL`): a theft out of another actor's possession in the same room; reports `if.event.taken` with `fromLocation` = victim, no messageId (the behavior narrates); silent refusal. `taking`'s scope rightly refuses an item inside another actor, which is what the thief hit first.
- Chain finding: 267/268 through 7 transcripts, then `wt-07` diverges because the thief's takes in DARK rooms are refused (`scope.not_visible`, `VisibilityBehavior.canSee:204`) and his state machine drifts. David's ruling: seeing in the dark is its own trait → `NightVisionTrait` (`packages/world-model/src/traits/night-vision/`, `TraitType.NIGHT_VISION = 'if.trait.night_vision'`, both barrels, `all-traits.ts`), honored at the two darkness gates in `VisibilityBehavior` (`canSee`, `getVisible`); Dungeo's five underground NPCs carry it.
- **Left broken on purpose at finalize**: `packages/world-model/src/traits/implementations.ts:84` needs a `NIGHT_VISION: NightVisionTrait` entry (the trait registry is `Record<TraitType, …>` — a registration point beyond the two barrels; add to the world-model memory). `tests/unit/world/night-vision.test.ts` declares darkness as `RoomTrait({ isDark: true })` but the idiom is `requiresLight: true` (3 of 4 cases fail until fixed; core-concepts' `isDark` example is stale). Both are one-liners; the last chain run (267/268) predates the trait and is not evidence for it.
- Dungeo unit tests: 46 passing (7 files) at the pre-NightVision build.

## Phase 6a/6b completion (23:10–00:05 CDT, after the finalize push)
- The two one-liners (`implementations.ts` NIGHT_VISION entry; `requiresLight: true` in the test) → world-model 1496 passing (+4 night-vision) → full build green.
- Chain diagnosis, three rounds, each an explained finding answered at its own layer: (1) the thief's darkness — NightVisionTrait (already in); (2) the platform's witnessed-mover line "The seedy-looking thief enters from the southwest." — David: "we're adding non-corpus text to dungeo? when did that come up?" → it was Phase 5 call 7, which had flipped the old opt-in default to always-on; `going` now gates on `NpcTrait.announcesMovement` (Chord's `announces-movement` still lowers to it) — going-witnessed test +1 case, ADR-203 AC-1/2 and the engine actor-turn test opt their NPCs in; (3) the thief fleeing the spinning Round Room drew `dungeo.round-room.exit` under the player's next `south` → `carousel-exit-resolver.ts` defers a non-player mover to static topology (MDL: CAROUSEL-EXIT is the player's exit function; ROBBER never traverses exits).
- **Chain: 952 passing across 17 transcripts, every golden matched — no re-pin** (23:55 CDT). stdlib 1661, story-loader 971, engine actor-turn 7, Dungeo 46 — all passing.
- Dungeo unit transcripts: 1733 passing, 9 failing, 10 expected failures, 2 skipped (one further failure, `implicit-take-put` "gold card", appeared in one of two runs — unpinned seed). NPC-shaped: `troll-interactions` asserts the troll's blow as a `game.message` event (now `if.event.attacked` with the same prose); `wave-rainbow` asserts `[EVENT: false, type="if.event.actor_moved"]` on a blocked `east` and now sees an NPC's real move that turn. Baseline at `593945a8` in a worktree to separate pre-existing from explained.
- Docs: plan Phases 6a/6b DONE with evidence, 6c CURRENT; ADR-328 D5 corrected (opt-in narration) and D6 landing note; core-concepts NightVision line + `requiresLight` idiom fix.
- Baseline (`593945a8` worktree): unit transcripts 1742/1 (`info-channel-baseline` pre-existing). One real regression found: `taking` awarded ADR-129 take-points to any actor → the thief's room thefts scored for the player (`trophy-case-scoring`, unpinned seed). Fixed: `taking.ts` guards the award on the protagonist; `taking-points-actor.test.ts` (2). Chain re-run: 952/17 byte-identical. Unit transcripts final 1738/5 (pre-existing 1, flake 1, explained 3 — David's ruling owed on editing `troll-interactions.transcript:75,:80` and `wave-rainbow.transcript:42,:76`).
- mutation-verification (Phase 6): 7 findings, 5 story-side gaps → closed with `stealing-action.test.ts` (4), `thief-lair-deposit.test.ts` (2), `troll-axe-behaviors.test.ts` (2), `carousel-exit-resolver.test.ts` (2), melee forced STAGGER/LOSE_WEAPON (+2). Dungeo vitest 58 passing; stdlib 1663.
