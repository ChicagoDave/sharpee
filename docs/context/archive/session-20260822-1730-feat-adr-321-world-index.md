...
**Progressive update (18:50 CDT) — Phase 6 (#288) DONE; plan DONE.** Gates re-derived for nesting: New Import from any active Chord source (story or fragment), import line at the caret's line; Extract snaps to whole declarations (column-0 structure, `##` runs as one construct), refuses the `story` header or an empty/blank selection, no longer refuses `import` lines. `tools/ide`: new `Editor/ImportRefactor.swift` (pure), `Editor/ImportCommands.swift` (prompt-free mutation), MainWindow prompts/forwarders, AppDelegate actions + validateMenuItem, MenuBuilder File → New Import… (⇧⌘N) and Edit → Extract Selection to Import…, editor `activeSelection`. Tests: `ImportRefactorTests` ×14, `EditorImportCommandTests` ×5; `xcodebuild test -derivedDataPath ./DerivedData` 589 passing, 0 failures. Two first-run failures were a classifier gap (consecutive `##` lines are one construct — fixed) and a wrong premise (editor clears undo on tab switch — pre-existing, test dropped, limit documented). #287 closed after 004147f2. `docs/work/tier-2-import-seam/plan.md` all six phases DONE, `**Plan Status**: DONE` — archive after this commit.
