...
- 2026-09-04 ~06:45 CDT: David hand-published `@sharpee/world-index@5.3.0` and `@sharpee/ext-chapters@5.3.0` (both `public` at the auth layer; the ext-chapters packument lagged the CDN ~5 min). On his word, dispatched the REAL release: `publish-npm.yml --ref main -f dry_run=false`, run 33869141138 on 454fc347.
- Run 33869141138 FAILED at `@sharpee/channel-service` (E404 on PUT after a 5-min stall; the package exists at 5.1.1 and published via this workflow on 2026-08-19 — transient, not config). Landed at 5.3.0 before the stop: core, chord, if-domain, ide-protocol, lang-en-us, media, text-blocks, world-model (+ the two hand-published). Re-dispatched; `--changed` resumes at channel-service.
- Re-dispatch 33870106653 SUCCESS: the remaining 24 packages published at 5.3.0. **Sharpee 5.3.0 / Chord 3.6.0 is on npm** — 34 packages (8 from the first run, 24 from the resume, 2 by David's hand). Plan Phase 18's npm half is done; Chord Writer's release and the install-page update remain.
