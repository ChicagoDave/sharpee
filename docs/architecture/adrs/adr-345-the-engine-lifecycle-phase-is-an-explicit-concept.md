...
**D2 — Each lifecycle method names the phases it accepts, and one refusal reports the phase it found.** `installStory` accepts `empty` only; `start` accepts `ready`; `resume` accepts `stopped`; `executeTurn` accepts `playing`. The guard-order question dissolves rather than being answered: there is one guard per method, so there is no order.
