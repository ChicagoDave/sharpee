...
**Amended 2026-09-11 (session c3f995) — D9 and D15 each carry a pointer to ADR-347**, which was accepted after this ADR's own amendment below. D9 gains a note that ADR-347 D2 answers a question D9 never asked (who owns the *ending*, as against where the stop *reason* lives); D15 gains the record that the `loadSaveData` / `undo()` resume patch its implementation needed has been **retired** by ADR-347 D3 rather than promoted to a rule here. Neither changes a decision; both exist so a reader does not infer one this ADR never made. Written in the same commit as the code, per ADR-347 D4b, gated by its AC-8.

**Amended 2026-09-11 (session aceb2e, David: "amend 345") — D15 and AC-10 added**, narrowing what the `stopped` phase refuses so an end-game prompt can answer `restart`, `restore`, `quit` and `undo` (GH #414, reported from the field). The amendment adds no phase and no transition; D10's closed set is unchanged. Implementation is scheduled as its own plan and is not authorized by the amendment alone.
...
**Amended 2026-09-11 (D15): `executeTurn` accepts `playing`, and accepts the meta route in `stopped`.** The sentence above is otherwise unchanged — the refusal is still one guard naming the phase it found; what changed is that in `stopped` it asks the route first.
...
**Amended 2026-09-11 (session c3f995) — a pointer, not a correction: ADR-347 answers a question D9 did not ask.** D9 settled where the *reason* for stopping lives (the event streams, not the phase) and it stands. What it assumed without asking is that `stopped` is a fact about the engine. ADR-347 D2 rules that the **ending** is a fact about the *world* — a first-class `WorldModel` member, `IStoryEnding` — which is why it outlived the engine that observed it across a restore. The two are not the same fact: `stopped` answers "may the engine take a turn", the Ending answers "did this story reach a conclusion", and ADR-347 D4 keeps `stopped` deliberately overloaded rather than splitting it. So D9's "engine state is the record of *what phase*" is unchanged, and the record of *what ending* is now somewhere else entirely. See ADR-347 D2, D2a, D4.
...
**Unchanged by the 2026-09-11 amendment.** D15 lets a meta command run while `stopped`; it adds no fifth state, no back-edge, and no in-place rebuild. An engine that answers `restart` at the end-game prompt still stops and is still discarded — the client still builds the fresh one.
...
**Amended by the implementing session (2026-09-10, session cbdf63, Phase 3): there is a third condition, and it is the earliest one.** The trace above starts at the `switch`, but `tryProcessDomainEventMessage` runs *before* it (`pipeline.ts:313`) and renders any event whose payload carries a `messageId`, whatever its type — the check is `if (!data?.messageId) return null` (`handlers/domain-message.ts:52`), nothing about `game.*`. So the full condition is that the payload carries no `message`, no `text`, **and no `messageId`**, plus no registered template. The decision is unchanged; only its stated evidence was incomplete, and a later payload change adding a `messageId` for some consumer's benefit would silently start rendering prose. All three fields are asserted absent by AC-5's second test rather than left to the trace.

**D15 — A meta command is accepted in the `stopped` phase; a world command is not** (David, 2026-09-11, session aceb2e, on GH #414: "amend 345"). `executeTurn`'s guard refuses *every* turn once the phase is `stopped` (`game-engine.ts:908-914`), and meta-command handling is a turn *stage* (`turn/meta-command.ts`), so it sits behind that guard. The consequence, reported from the field by a player who reached an ending in the browser: `restart`, `restore`, `quit` and `undo` are all equally dead — the four verbs an IF player reaches for at exactly that moment, and the reason a finished story became an unrecoverable one. All four are already registered meta commands (`stdlib/src/actions/meta-registry.ts:24-41`); nothing about them is new, and nothing about them wants a live world.

**This narrows what `stopped` refuses. It does not add a phase, a transition, or a reason to read one.** D10 stands unamended: a meta command run while stopped leaves the engine stopped, and restart remains engine disposal the client performs by building a fresh engine.

**The seam already exists, which is why this is a narrowing rather than a design.** The route is decided at the parse stage (`turn/parse.ts:37`, via `MetaCommandRegistry.isMeta`), which both stage lists share, and the meta list is `SHARED_STAGES` plus `metaCommandStage` and `metaRenderStage` (`turn/stages.ts:83-88`). It reaches neither `advanceTurnStage`, `pluginTickStage` nor `endingStage`, so a meta command cannot advance the turn, tick a daemon, or re-enter the ending path that stopped the engine in the first place. The refusal therefore moves from *before any stage* to *the route switch*: `stopped` runs the shared stages, accepts the meta route, and refuses the turn route naming the phase it found, as D2 requires.

**Why the engine rather than each client.** The client-side alternative — a post-game input mode in `BrowserClient` handling the meta verbs without calling `executeTurn` — needs no ADR amendment and was rejected on two grounds: it duplicates into every host that runs an engine (CLI, IDE Play pane, browser, bridge, runtime), and it cannot do `undo` at all, since undo is an engine operation with no client-side equivalent. One rule in the turn cycle serves every host at once.

**Amended 2026-09-11 (session c3f995) — the restore and undo seam is ADR-347 D3's, and this ADR states no standing rule about it.** Implementing D15 surfaced a second defect one level down: `loadSaveData` replaced the world without touching the phase, so a RESTORE at an end-game prompt loaded a *live* save into an engine that still refused every command. Phase 1 of the implementing plan patched it as `if (this.phase.name === 'stopped') this.resume()` in `loadSaveData` and `undo()`, and recorded that the rule wanted a D15 amendment line if it were to stand.

**It does not stand, and the line it asked for is this one.** ADR-347 D3 rules that the phase is **derived** from the restored world at that seam: a save carrying an Ending leaves the engine stopped, a save without one returns it to play (`GameEngine.derivePhaseFromEnding`, landed 2026-09-11). That replaced the patch rather than joining it — the unconditional resume is deleted, which ADR-347 AC-5 verifies by grep. D15 is therefore unamended in substance: it says what `stopped` refuses, and says nothing about what a restore does to the phase. **Read no standing resume rule into it** — the patch existed for one phase of one plan and is gone.

D10's closed set is still untouched by this: `playing → stopped` is `stop()`'s own edge and `stopped → playing` is `resume()`'s, and the derivation uses only those two.

**What this does NOT decide.** The client half of GH #414 is client behavior, not engine lifecycle, and is **not** authorized by this amendment: the per-turn autosave fires on the ending turn (`platform-browser/src/BrowserClient.ts:435`) while a Chord ending lives in world state (`story-loader/src/loader.ts:1512-1514`), so a page reload restores an already-ended world and the engine re-stops every turn; and no client subscribes to `game:over`, so the input box stays live after the story ends with no end-game prompt. Both are ruled in the implementing plan, under the platform-approval rule, not here.

**What would falsify this**: a meta command whose action mutates the world or depends on a live turn cycle. Surveyed 2026-09-11 by reading each `execute()` in the registry's default set (`meta-registry.ts:128-140`), and **the survey found one hit, so this is a boundary rather than a clean bill of health**:

- `saving`, `restoring`, `quitting`, `restarting`, `undoing` write no world state of their own — each emits a platform request, and `restoring` and `undoing` replace the world wholesale through theirs rather than mutating the stopped session.
- `version` and `about` have empty `execute()` bodies, both saying so in a comment. `scoring` reads `world.getScore()` / `getMaxScore()` / `getRank()` and writes nothing.
- **`help` mutates.** `help.ts:130-135` registers the `GAME_META` capability if absent and sets `helpRequested: true` on it — capability data that serializes with the world, as its own comment records. So HELP at an end-game prompt writes to a stopped session's world.
- `verbose`, `brief`, `superbrief` and `notify` are registered as meta (`meta-registry.ts:139`) but have **no action directory** under `actions/standard/`; there is nothing to survey, and `metaCommandStage` would answer `action_not_found` for them. Noted as found, not fixed here.

`help`'s write is harmless on the merits — a flag recording that help was asked for, on a world the next restart discards — and it does not change the decision. It does mean the rule is "a meta command may run while stopped", not "a meta command cannot touch the world", and the difference matters for the next one added. A story registering its own meta command (`MetaCommandRegistry.register`, `meta-registry.ts:63`) inherits the allowance without asking for it, so the plan owes that boundary a stated answer rather than a silent assumption.
...
9. **AC-10 (D15) — the end-game verbs answer, and world commands still do not.** A test drives `start` → `stop('defeat')`, then asserts `executeTurn('score')` resolves rather than throwing, and that `executeTurn` on a world command still refuses naming `'stopped'`. A second assertion pins that the meta command changed nothing about the lifecycle: the engine is still `stopped` afterwards, and no `game.resumed` was emitted. **SELF-VERIFYING** — deleting the narrowing fails the first assertion, and widening it to every command fails the second.
...
- **An ending stops being a dead prompt** (D15). `stopped` refuses world commands rather than all commands, so RESTART / RESTORE / QUIT / UNDO answer at the end-game prompt for every host at once instead of each client growing its own post-game input mode. The honest cost is that `stopped` is now two rules where it was one, and a story that registers a custom meta command inherits the allowance without having asked for it.
