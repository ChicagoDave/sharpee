...
This ADR obliges no new behavior, so its criteria are mostly *already discharged* — they name the tests that make D1 through D4 true today, so a later change that breaks one is caught rather than merely disagreed with. Two are not discharged, and are marked.
...
4. **AC-4 (D3's boundary: the seam, not the turn loop) — NOT DISCHARGED by a unit test, and guarded in integration.** Nothing directly pins that `derivePhaseFromEnding` runs only at the two seams. The guard that exists is indirect but real: moving the call into the turn loop would re-stop the engine the branch-tester's walker had just revived, failing every tree whose line follows a death card. A unit test asserting an engine stays `playing` for a full turn while the world carries an Ending would make the property local instead of emergent. Recorded as a gap rather than claimed as coverage.
