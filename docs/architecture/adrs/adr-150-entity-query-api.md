...
// After
const lightSources = w.contents(roomId)
  .withTrait(LightSourceTrait)
...
**Forward declarations for ADR-149**: The `w.scenes`, `w.regions`, `inRegion()`, and `withinRegion()` methods reference `EntityType.REGION`, `EntityType.SCENE`, `RegionTrait`, and `SceneTrait`. This package declares the entity type constants and minimal trait type stubs so the query methods compile. ADR-149 provides the full trait implementations — the stubs here are just enough for type checking and filtering. Until ADR-149 is implemented, `w.scenes` and `w.regions` return empty queries (no entities of those types exist).
...
```
Given: WorldModel with room 'r01' (name: "Armory") containing entities
       'y01' (name: "sword", traits: [identity, weapon]) and
       'y02' (name: "shield", traits: [identity, wearable])
When:  import '@sharpee/queries'
Then:
  w.rooms.count()                                     === 1
...
This exercises: prototype augmentation, entry points (`w.rooms`, `w.contents()`), chaining (`named().single()`), aggregation (`count`, `any`, `none`), materialization (`toIdSet`), and rejection (`single()` on empty).

## Acceptance Criteria

1. `@sharpee/queries` package builds and exports `EntityQuery`
2. All methods from the EntityQuery class definition are implemented
3. Augmentation works on `WorldModel` at runtime (not just type-level)
4. End-to-end test scenario (Section 7) passes
5. Existing platform and story test suites pass unchanged
6. At least one real story-code site (Dungeo or Fever Dream) refactored to use queries as a smoke test
