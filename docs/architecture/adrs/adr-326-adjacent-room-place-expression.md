...
**D5 addendum, corrected (2026-09-06, GH #367, secret-letter-port-platform-defects plan Phase 2; David: walked parity).** The order the 2026-09-03 addendum promised did not hold: the arrival clauses' narration drained with the enclosing clause's own report while the description waited for the flush, so the reaction printed above the room. Both now ride the act queue, description first, and *every* level of a re-entry chain describes its room before that room's clauses run — exactly as each walked arrival of a bounce does — so the blocked-stall bounce narrates the stall, the keeper's yell, then the room the player lands in. The "outermost only" sentence above is withdrawn. The full order is `docs/architecture/authored-move-narration-and-event-order.md`.
