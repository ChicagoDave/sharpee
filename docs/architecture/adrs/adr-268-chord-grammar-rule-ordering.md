...
## Status: ACCEPTED (2026-07-26) — specificity experiment (A7) run first and decisive (zero divergences, static + full transcript corpus; `docs/work/chord-grammar-ordering/specificity-experiment.md`); all three open questions resolved via interview same day (Q-1 no override construct, Q-2 tier-first, Q-3 engine swap now + `.withPriority` removal); adr-review 14/14 after two SMALL fixes (D5 tier contract; D5 full module list incl. if-domain sortRules removal). ADR-266 Q-7 answered: no ordering notation. Not yet implemented.
