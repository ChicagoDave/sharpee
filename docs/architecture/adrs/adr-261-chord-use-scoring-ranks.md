...
> **AMENDED 2026-07-24 (session 7f133e) — this decision is reversed by ADR-262 D3.** When the rank
> ladder was generalized onto ADR-262's banded-crossing engine, the owner chose **silence must be
> explicit**: every announce mode speaks by default via an *overridable* fallback line (a lang-en-us
> message id, `override message`-able), and a meter is quiet only under the `announce silent` mode.
> That is precisely the "platform default in lang-en-us" this D7 rejected — un-rejected, because
> ranks now share the engine and the two must not disagree. **So, as of ADR-262:** a rung with no
> `says` renders the overridable platform fallback, *not* silence; the lang-en-us rank-promotion
> default rejected above now exists; and `if.event.rank_risen` is superseded by the generic
> `if.event.band_crossed` (ADR-262 D2). D7's design reasoning stands as the record of why silence-first
> was tried; the operative behavior is ADR-262 D3.
