...
- **A specific implementation technology** (Electron vs Tauri vs native). Implementation choice is deferred to the implementation plan. — **CLOSED as native on both desktop platforms** by [ADR-341](./adr-341-chord-writer-windows.md) (macOS: native Swift/AppKit, recorded retroactively; Windows: native C# on .NET, D2), with the spike evidence in `docs/work/archive/adr-341-spike/decision.md` (2026-09-12, session `89f9e0`). Electron and Tauri are both rejected there; the WPF-vs-WinUI-3 choice *within* native is a separate, narrower question settled by that same record.
