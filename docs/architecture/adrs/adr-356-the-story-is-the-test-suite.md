...
**Measured (2026-09-26, session 00004f, GH #520's plan Phases 1–4).** The End-to-End Scenario above counted fernhill's branches by clause and estimated 39; the enumerator D1 specifies counts leaf paths — each guard's refusal, each `select` arm, each ordinal — and measures **63**. The first cut (D2's floor, D3's runner, D5's branches ratio under `sharpee test`) exercises 33 of the 63 on fernhill, with 2 failures (one finding: an entity-keyed topic answers with Tobias's `on asking` fallback when the entity is out of scope) and 30 named SKIPPED shapes. On secret-letter: 721 branches, 358 exercised, 12 failures, 363 SKIPPED by shape (the list that decides which shape the arrange step learns next). The full report and the shape counts are in `docs/work/testing-explorer/plan-20260925-520-adr356-first-cut.md`, Phase 4.
