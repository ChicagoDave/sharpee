...
### Using the bundle (Sharpee development)
...
Stories can implement their own debug tools. For example, Dungeo implements a Game Debugging Tool (GDT) with teleport and take commands — but that's story-specific, not part of Sharpee's core testing infrastructure.
...
- Use `$teleport` and GDT to set up the scenario quickly
...
For manual playtesting outside of automated transcripts, see the original playtesting procedure below.

### Playtest Transcripts

Raw playtest output is saved to `docs/testing/tr-NNN.txt` with inline tester comments:
...
Each `tr-NNN-fixes.md` documents issues found and their resolutions. See existing files in `docs/testing/` for examples.

### Issue Categories

| Category | Scope | Examples |
|----------|-------|----------|
| **Platform** | engine/stdlib/world-model | Action bugs, trait behavior |
| **Map** | Room connections | Missing exits, wrong directions |
| **Puzzle** | Game logic | Dam mechanics, scoring |
| **Parser** | Grammar/parsing | Command not recognized |
| **Text** | Language layer | Wrong messages |
| **Client** | Terminal/browser | Display issues |
