...
1. **`config`**: metadata such as the story's title, author, version, and ID. The
...
**The scaffolded `src/index.ts` isn't written exactly like the file we build here,
but both are valid.** The stub imports `Story` and `StoryConfig` from
...
`world.moveEntity(entityId, locationId)`, which puts the entity *inside* the
