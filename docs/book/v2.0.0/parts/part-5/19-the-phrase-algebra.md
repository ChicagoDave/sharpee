...
`{a item}` and `{an item}` mean the same thing. Both select *indefinite*, and
the Assembler picks the actual surface word: "a flashlight," "an owl," even
...
is it a proper name, a mass noun, or a plural? That information lives on the entity
...
entirely), and its adjectives. Every hint and agreement feature downstream reads
that metadata. This is the practical reason the earlier chapters were careful about
...
`{target}` bound to a string defaults to *indefinite* and renders "a toucan," and
worse, a whole sentence bound as a string gets an article stuck on the front ("a
You hear footsteps…"). Prose that should pass through untouched needs
`{verbatim:…}`, below. A number or boolean bound bare is inserted as-is: you get
"42," not "a 42."
...
`opens`, `refuses`) and the Assembler conjugates it: "the door opens" but "the
...
producing "a north" or "a Aragorn." `{verbatim:text}` passes the value through
byte-for-byte; even internal spacing and line breaks survive. The platform's own
room description template is built on `{verbatim:description}`: your room prose
is already written, and nothing should touch it.
...
> `engine.registerSlotContributor`, covered in the platform reference.
...
The matching template is `The staff gate is set into the fence{openClause}.`
When `present` is false the clause vanishes, along with its comma, which is
absorbed cleanly rather than left dangling.
...
- **Legacy colon chains** such as `{the:item}` or `{the:cap:item}`: "`'the'` is
  not a known kind prefix — legacy ':' chains are not supported." The pre-2.0
  formatter syntax is a deliberate clean break, not a quiet deprecation. (The
  dash in that message is the parser's own text.)
...
variation never go in the string; you build `Optional` and `Choice` values in code
