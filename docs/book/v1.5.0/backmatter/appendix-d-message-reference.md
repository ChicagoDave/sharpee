...
| Message ID | Default text |
|---|---|
| `character.propagation.witnessed.conspiratorial` | {speakerName} leans close to {listenerName}, muttering under their breath. |
| `character.propagation.witnessed.dramatic` | {speakerName} excitedly tells {listenerName} about something. |
| `character.propagation.witnessed.fearful` | {speakerName} nervously whispers something to {listenerName}. |
...
| Message ID | Default text |
|---|---|
| `if.action.about.about_compact` | {title} v{version} by {author} |
| `if.action.about.about_footer` | Thank you for playing! |
| `if.action.about.about_header` | About {title} |
...
| `if.action.about.description` | Description: {description} |
| `if.action.about.engine_info` | Powered by {engine} version {engineVersion} |
| `if.action.about.enjoy_game` | We hope you enjoy playing {title}! |
| `if.action.about.game_info` | {title} Version {version} By {author} Released: {releaseDate} |
| `if.action.about.game_info_simple` | {title} by {author} |
...
| Message ID | Default text |
|---|---|
| `if.action.asking.already_told` | {the:cap:target} says, "I already told you about that." |
| `if.action.asking.confused` | {the:cap:target} looks confused. |
| `if.action.asking.earned_trust` | {the:cap:target} says, "Since you've proven yourself, I'll tell you..." |
| `if.action.asking.explains` | {the:cap:target} explains about {topic}. |
| `if.action.asking.must_do_first` | {the:cap:target} says, "There's something you need to do first." |
| `if.action.asking.no_idea` | {the:cap:target} says, "No idea what you're talking about." |
...
| `if.action.asking.not_yet` | {the:cap:target} says, "I can't tell you about that yet." |
| `if.action.asking.remembers` | {the:cap:target} says, "Ah yes, about {topic}..." |
| `if.action.asking.responds` | {the:cap:target} tells you about {topic}. |
| `if.action.asking.shrugs` | {the:cap:target} shrugs. |
...
| Message ID | Default text |
|---|---|
| `if.action.attacking.already_dead` | {the:cap:target} is already dead. |
...
| `if.action.attacking.defends` | {the:cap:target} defends against {your} attack. |
| `if.action.attacking.destroyed` | {You} {destroy} {the:target}! |
| `if.action.attacking.dodges` | {the:cap:target} dodges {your} attack. |
| `if.action.attacking.flees` | {the:cap:target} flees from {you}! |
...
| `if.action.attacking.indestructible` | {the:cap:target} is far too solid to damage. |
| `if.action.attacking.items_spilled` | {the:cap:target}'s possessions spill onto the ground. |
...
| `if.action.attacking.need_weapon_to_damage` | {the:cap:target} requires a weapon to damage. |
...
| `if.action.attacking.retaliates` | {the:cap:target} fights back! |
| `if.action.attacking.self` | Violence against {yourself} isn't the answer. |
| `if.action.attacking.shattered` | {the:cap:target} shatters! |
...
| `if.action.attacking.target_broke` | {the:cap:target} breaks! |
| `if.action.attacking.target_damaged` | {the:cap:target} shows signs of damage. ({damage} damage dealt) |
| `if.action.attacking.target_destroyed` | {the:cap:target} is utterly destroyed! |
| `if.action.attacking.target_shattered` | {the:cap:target} shatters into pieces! |
...
| Message ID | Default text |
|---|---|
| `if.action.attacking.combat.already_dead` | {the:cap:target} is already dead. |
...
| `if.action.attacking.combat.not_hostile` | {the:cap:target} isn't hostile. |
...
| Message ID | Default text |
|---|---|
| `if.action.attacking.combat.defend.blocked` | {the:cap:target} blocks {your} attack! |
...
| Message ID | Default text |
|---|---|
| `if.action.attacking.combat.health.badly_wounded` | {the:cap:target} is badly wounded. |
| `if.action.attacking.combat.health.dead` | {the:cap:target} is dead. |
| `if.action.attacking.combat.health.healthy` | {the:cap:target} appears uninjured. |
| `if.action.attacking.combat.health.near_death` | {the:cap:target} is barely clinging to life! |
...
| `if.action.climbing.cant_go_that_way` | {You} {can't} climb {direction} from here. |
...
| Message ID | Default text |
|---|---|
| `if.action.closing.already_closed` | {the:cap:item} is already closed. |
...
| `if.action.entering.container_closed` | {the:cap:container} is closed. |
...
| `if.action.entering.occupied` | {the:cap:place} is already occupied. |
...
| Message ID | Default text |
|---|---|
| `if.action.examining.brief_description` | {description} |
| `if.action.examining.cant_see` | {You} {can't} see {the:item} here. |
| `if.action.examining.container_closed` | {the:cap:item} is closed. |
| `if.action.examining.container_contents` | In {the:container} {you} {see} {items}. |
| `if.action.examining.container_empty` | {the:cap:item} is empty. |
| `if.action.examining.container_open` | {the:cap:item} is open. |
| `if.action.examining.description` | {description} |
| `if.action.examining.examined` | {description} |
| `if.action.examining.examined_container` | {description} |
| `if.action.examining.examined_door` | {description} |
| `if.action.examining.examined_readable` | {description} |
| `if.action.examining.examined_self` | {description} |
| `if.action.examining.examined_supporter` | {description} |
| `if.action.examining.examined_switchable` | {description} |
| `if.action.examining.examined_wall` | {description} |
| `if.action.examining.examined_wearable` | {description} |
...
| `if.action.exiting.container_closed` | {the:cap:container} is closed. |
...
| Message ID | Default text |
|---|---|
| `if.action.giving.accepts` | {the:cap:recipient} accepts {the:item}. |
| `if.action.giving.given` | {You} {give} {the:item} to {the:recipient}. |
| `if.action.giving.gratefully_accepts` | {the:cap:recipient} gratefully accepts {the:item}. |
| `if.action.giving.inventory_full` | {the:cap:recipient} says, "I can't carry any more." |
...
| `if.action.giving.not_interested` | {the:cap:recipient} doesn't seem interested in {the:item}. |
| `if.action.giving.recipient_not_reachable` | {the:cap:recipient} is too far away. |
| `if.action.giving.recipient_not_visible` | {You} {can't} see {the:recipient}. |
| `if.action.giving.refuses` | {the:cap:recipient} politely declines. |
| `if.action.giving.reluctantly_accepts` | {the:cap:recipient} reluctantly takes {the:item}. |
...
| `if.action.going.door_closed` | {the:cap:door} is closed. |
| `if.action.going.door_locked` | {the:cap:door} is locked. |
| `if.action.going.moved` | {You} {go} {direction}. |
| `if.action.going.movement_blocked` | {message} |
...
| `if.action.going.room_description` | {name} {description} |
...
| Message ID | Default text |
|---|---|
| `if.action.help.first_time_help` | New to Interactive Fiction? Try these commands to get started: - LOOK to see where you are - INVENTORY to see what you're carrying - EXAMINE interesting objects - Go in compass directions (NORTH, SOUTH, etc.) |
| `if.action.help.general_help` | Welcome to Interactive Fiction!  Basic commands: - LOOK (L): Examine your surroundings - INVENTORY (I): List what you're carrying - EXAMINE (X) [object]: Look at something closely - TAKE/DROP [object]: Pick up or put down items - GO [direction] or just [direction]: Move around  For more help on a specific topic, type HELP [topic]. |
...
| `if.action.help.help_topic` | Help on {topic}: |
...
| Message ID | Default text |
|---|---|
| `if.action.inserting.already_there` | {the:cap:item} is already in {the:destination}. |
| `if.action.inserting.container_closed` | {the:cap:container} is closed. |
...
| `if.action.listening.device_off` | {the:cap:target} is silent. |
| `if.action.listening.device_running` | {the:cap:target} is making a soft humming sound. |
...
| `if.action.listening.no_sound` | {the:cap:target} isn't making any sound. |
...
| Message ID | Default text |
|---|---|
| `if.action.locking.already_locked` | {the:cap:item} is already locked. |
...
| `if.action.looking.room_description` | {name} {description} |
...
| Message ID | Default text |
|---|---|
| `if.action.opening.already_open` | {the:cap:item} is already open. |
| `if.action.opening.cant_reach` | {You} {can't} reach {the:item}. |
| `if.action.opening.its_empty` | {You} {open} {the:container}, which is empty. |
| `if.action.opening.locked` | {the:cap:item} is locked. |
| `if.action.opening.no_target` | Open what? |
| `if.action.opening.not_openable` | {the:cap:item} can't be opened. |
...
| `if.action.pulling.fixed_in_place` | {the:cap:target} is fixed in place. |
...
| `if.action.pulling.pulled_direction` | {You} {pull} {the:target} {direction}. |
| `if.action.pulling.pulled_nudged` | {You} {tug} at {the:target}, moving it slightly. |
| `if.action.pulling.pulled_with_effort` | With effort, {you} {drag} {the:target} {direction}. |
| `if.action.pulling.pulling_does_nothing` | Pulling {the:target} has no effect. |
| `if.action.pulling.too_heavy` | {the:cap:target} is too heavy to pull (weighs {weight}kg). |
...
| `if.action.pushing.fixed_in_place` | {the:cap:target} is fixed in place. |
...
| `if.action.pushing.pushed_direction` | {You} {push} {the:target} {direction}. |
| `if.action.pushing.pushed_nudged` | {You} {give} {the:target} a push, but it doesn't move far. |
| `if.action.pushing.pushed_with_effort` | With considerable effort, {you} {push} {the:target} {direction}. |
| `if.action.pushing.pushing_does_nothing` | Pushing {the:target} has no effect. |
| `if.action.pushing.reveals_passage` | As {you} {push} {the:target} {direction}, it slides aside, revealing a hidden passage! |
| `if.action.pushing.switch_toggled` | {You} {push} {the:target}, toggling it {newState}. |
| `if.action.pushing.too_heavy` | {the:cap:target} is far too heavy to push (weighs {weight}kg). |
...
| Message ID | Default text |
|---|---|
| `if.action.putting.already_there` | {the:cap:item} is already {relation} {the:destination}. |
...
| `if.action.putting.container_closed` | {the:cap:container} is closed. |
...
| Message ID | Default text |
|---|---|
| `if.action.reading.cannot_read_now` | {reason} |
| `if.action.reading.not_readable` | There's nothing written on {the:item}. |
| `if.action.reading.read_book` | {the:cap:item} reads: {text} |
| `if.action.reading.read_book_page` | {the:cap:item} (page {currentPage} of {totalPages}): {text} |
| `if.action.reading.read_inscription` | {the:cap:item} reads: {text} |
| `if.action.reading.read_sign` | {the:cap:item} says: {text} |
...
| `if.action.removing.container_closed` | {the:cap:container} is closed. |
| `if.action.removing.no_source` | Remove {the:item} from what? |
| `if.action.removing.no_target` | Remove what? |
| `if.action.removing.not_in_container` | {the:cap:item} isn't in {the:container}. |
| `if.action.removing.not_on_surface` | {the:cap:item} isn't on {the:surface}. |
...
| `if.action.restoring.confirm_restore` | Restore game from '{saveName}'? Current progress will be lost. |
| `if.action.restoring.corrupt_save` | The save file '{saveName}' appears to be corrupted. |
| `if.action.restoring.game_loaded` | Game loaded from '{saveName}'. |
...
| `if.action.restoring.restore_details` | Restored: {saveName} Score: {score} Moves: {moves} |
...
| `if.action.restoring.save_not_found` | No save named '{saveName}' was found. |
...
| `if.action.saving.confirm_overwrite` | A save named '{saveName}' already exists. Overwrite it? |
| `if.action.saving.game_saved` | Game saved. |
| `if.action.saving.game_saved_as` | Game saved as '{saveName}'. |
| `if.action.saving.invalid_save_name` | '{saveName}' is not a valid save name. |
| `if.action.saving.no_save_slots` | No save slots available. |
| `if.action.saving.overwrite_save` | Previous save '{saveName}' has been overwritten. |
| `if.action.saving.quick_save` | Quick save completed. |
| `if.action.saving.save_details` | Saved: {saveName} Score: {score} Moves: {moves} |
...
| `if.action.saving.save_slot` | Game saved to slot {saveName}. |
...
| Message ID | Default text |
|---|---|
| `if.action.searching.container_closed` | {the:cap:target} is closed. |
| `if.action.searching.container_contents` | In {the:target} {you} {see}: {items}. |
| `if.action.searching.empty_container` | {the:cap:target} is empty. |
...
| `if.action.showing.viewer_examines` | {the:cap:viewer} examines {the:item} carefully. |
| `if.action.showing.viewer_impressed` | {the:cap:viewer} looks impressed. |
| `if.action.showing.viewer_nods` | {the:cap:viewer} nods. |
| `if.action.showing.viewer_not_visible` | {You} {can't} see {the:viewer}. |
| `if.action.showing.viewer_recognizes` | {the:cap:viewer} recognizes {the:item}! |
| `if.action.showing.viewer_too_far` | {the:cap:viewer} is too far away to see clearly. |
...
| Message ID | Default text |
|---|---|
| `if.action.smelling.burning_scent` | {the:cap:target} gives off a smoky smell. |
| `if.action.smelling.container_food_scent` | {You} {smell} food inside {the:target}. |
| `if.action.smelling.drink_scent` | {the:cap:target} has a pleasant aroma. |
| `if.action.smelling.food_nearby` | {You} {smell} food nearby. |
| `if.action.smelling.food_scent` | {the:cap:target} smells delicious. |
| `if.action.smelling.fresh_scent` | {the:cap:target} smells fresh and clean. |
| `if.action.smelling.musty_scent` | {the:cap:target} smells a bit musty. |
| `if.action.smelling.no_particular_scent` | {the:cap:target} has no particular smell. |
...
| Message ID | Default text |
|---|---|
| `if.action.switching_off.already_off` | {the:cap:target} is already off. |
| `if.action.switching_off.device_stops` | {the:cap:target} powers down with a soft whir. |
| `if.action.switching_off.door_closes` | {the:cap:target} switches off and closes. |
...
| `if.action.switching_off.not_switchable` | {the:cap:target} isn't something {you} can switch off. |
...
| Message ID | Default text |
|---|---|
| `if.action.switching_on.already_on` | {the:cap:target} is already on. |
| `if.action.switching_on.device_humming` | {the:cap:target} hums to life. |
| `if.action.switching_on.door_opens` | {the:cap:target} switches on and opens. |
| `if.action.switching_on.illuminates_darkness` | {the:cap:target} switches on, banishing the darkness. |
| `if.action.switching_on.light_on` | {You} {switch} on {the:target}, illuminating the area. |
| `if.action.switching_on.no_power` | {the:cap:target} has no power source. |
| `if.action.switching_on.no_target` | Switch on what? |
| `if.action.switching_on.not_reachable` | {You} {can't} reach {the:target}. |
| `if.action.switching_on.not_switchable` | {the:cap:target} isn't something {you} can switch on. |
...
| `if.action.taking.fixed_in_place` | {the:cap:item} is fixed in place. |
...
| Message ID | Default text |
|---|---|
| `if.action.talking.acknowledges` | {the:cap:target} acknowledges {you}. |
| `if.action.talking.casual_greeting` | {the:cap:target} says, "Hey!" |
| `if.action.talking.first_meeting` | {You} {introduce} {yourself} to {the:target}. |
| `if.action.talking.formal_greeting` | {the:cap:target} says, "Good day to you." |
| `if.action.talking.friendly_greeting` | {the:cap:target} smiles in recognition. |
| `if.action.talking.greets_again` | {the:cap:target} says, "Hello again." |
| `if.action.talking.greets_back` | {the:cap:target} says, "Hello there!" |
| `if.action.talking.has_topics` | {the:cap:target} seems willing to discuss various topics. |
| `if.action.talking.no_response` | {the:cap:target} doesn't respond. |
| `if.action.talking.no_target` | Talk to whom? |
| `if.action.talking.not_actor` | {You} can only talk to people. |
| `if.action.talking.not_available` | {the:cap:target} doesn't want to talk right now. |
| `if.action.talking.not_visible` | {You} {can't} see {the:target}. |
| `if.action.talking.nothing_to_say` | {the:cap:target} has nothing particular to say. |
| `if.action.talking.remembers_you` | {the:cap:target} says, "Ah, it's you again." |
...
| Message ID | Default text |
|---|---|
| `if.action.telling.already_knew` | {the:cap:target} says, "Yes, I'm aware of that." |
| `if.action.telling.bored` | {the:cap:target} looks bored. |
| `if.action.telling.dismissive` | {the:cap:target} says, "So what?" |
| `if.action.telling.grateful` | {the:cap:target} says, "Thank you for telling me!" |
| `if.action.telling.ignores` | {the:cap:target} ignores what {you're} saying. |
| `if.action.telling.informed` | {You} {inform} {the:target} about {topic}. |
| `if.action.telling.interested` | {the:cap:target} listens with interest. |
...
| `if.action.telling.not_interested` | {the:cap:target} doesn't seem interested. |
| `if.action.telling.not_visible` | {You} {can't} see {the:target}. |
| `if.action.telling.told` | {You} {tell} {the:target} about {topic}. |
...
| Message ID | Default text |
|---|---|
| `if.action.throwing.bounces_off` | {the:cap:item} bounces off {the:target}. |
| `if.action.throwing.breaks_against` | {the:cap:item} smashes against {the:target}! |
| `if.action.throwing.breaks_on_impact` | {the:cap:item} shatters on impact! |
...
| `if.action.throwing.lands_in` | {the:cap:item} lands in {the:target}. |
| `if.action.throwing.lands_on` | {the:cap:item} lands on {the:target}. |
| `if.action.throwing.misses_target` | {You} {throw} {the:item} at {the:target}, but miss. |
| `if.action.throwing.no_exit` | There's no exit {direction}. |
| `if.action.throwing.no_item` | Throw what? |
| `if.action.throwing.not_holding` | {You} aren't holding {the:item}. |
| `if.action.throwing.sails_through` | {the:cap:item} sails through the exit to the {direction}. |
| `if.action.throwing.self` | {You} {can't} throw things at {yourself}. |
| `if.action.throwing.target_angry` | {the:cap:target} doesn't appreciate being hit with {the:item}. |
| `if.action.throwing.target_catches` | {the:cap:target} catches {the:item}! |
| `if.action.throwing.target_ducks` | {the:cap:target} ducks out of the way. |
| `if.action.throwing.target_not_here` | {the:cap:target} isn't here. |
...
| `if.action.throwing.thrown_direction` | {You} {throw} {the:item} {direction}. |
...
| Message ID | Default text |
|---|---|
| `if.action.touching.device_vibrating` | {the:cap:target} is vibrating slightly. |
| `if.action.touching.feels_cold` | {the:cap:target} feels cold. |
| `if.action.touching.feels_hard` | {the:cap:target} feels hard and solid. |
| `if.action.touching.feels_hot` | {the:cap:target} is hot! {You} {pull} {your} hand back quickly. |
| `if.action.touching.feels_normal` | {the:cap:target} feels as {you}'d expect. |
| `if.action.touching.feels_rough` | {the:cap:target} feels rough. |
| `if.action.touching.feels_smooth` | {the:cap:target} feels smooth. |
| `if.action.touching.feels_soft` | {the:cap:target} feels soft. |
| `if.action.touching.feels_warm` | {the:cap:target} feels warm to the touch. |
| `if.action.touching.feels_wet` | {the:cap:target} feels damp. |
| `if.action.touching.immovable_object` | {the:cap:target} is solid and immovable. |
...
| Message ID | Default text |
|---|---|
| `if.action.turning.cant_turn_that` | {the:cap:target} isn't something {you} can turn. |
| `if.action.turning.crank_turned` | {You} {crank} {the:target}. |
| `if.action.turning.dial_adjusted` | {You} {adjust} {the:target} {direction}. |
...
| Message ID | Default text |
|---|---|
| `if.action.unlocking.already_unlocked` | {the:cap:item} is already unlocked. |
...
| `if.action.unlocking.not_lockable` | {the:cap:item} can't be unlocked. |
| `if.action.unlocking.still_locked` | {the:cap:item} is locked. |
...
| Message ID | Default text |
|---|---|
| `if.action.version.version_compact` | {storyTitle} v{storyVersion} (Sharpee v{engineVersion}) |
