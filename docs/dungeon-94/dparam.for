...
C 
	PARAMETER (SDIR='40000'O)		! direct object present
	PARAMETER (SIND='20000'O)		! indirect object present
	PARAMETER (SSTD='10000'O)		! direct object std format
	PARAMETER (SFLIP='4000'O)		! flip direct, indirect obj
	PARAMETER (SDRIV='2000'O)		! default (driver) syntax
	PARAMETER (SVMASK='777'O)		! mask for verb number
	PARAMETER (VABIT='40000'O)		! search adv for object
	PARAMETER (VRBIT='20000'O)		! search room for object
	PARAMETER (VTBIT='10000'O)		! take object if possible
	PARAMETER (VCBIT='4000'O)		! must have object
	PARAMETER (VEBIT='2000'O)		! qualifier flags = fwim flags
	PARAMETER (VFBIT='1000'O)		! must reach object
	PARAMETER (VPMASK='777'O)		! mask for prep number
C
...
C Exit definitions
C
	PARAMETER (XLFLAG='100000'O)		! last entry flag
	PARAMETER (XDMASK='76000'O)		! direction mask
	PARAMETER (XRMASK='377'O)		! room mask
	...
	PARAMETER (XMIN='2000'O)		! minimum direction
	PARAMETER (XMAX='40000'O)		! maximum direction
	PARAMETER (XNORTH='2000'O)
	PARAMETER (XNE='4000'O)
	PARAMETER (XEAST='6000'O)
	PARAMETER (XSE='10000'O)
	PARAMETER (XSOUTH='12000'O)
	PARAMETER (XSW='14000'O)
	PARAMETER (XWEST='16000'O)
	PARAMETER (XNW='20000'O)
	PARAMETER (XUP='22000'O)
	PARAMETER (XDOWN='24000'O)
	PARAMETER (XLAUN='26000'O)
	PARAMETER (XLAND='30000'O)
	PARAMETER (XENTER='32000'O)
	PARAMETER (XEXIT='34000'O)
	PARAMETER (XCROSS='36000'O)
C
C Actor indices
C
...
C Actor flags
C
	PARAMETER (ASTAG='100000'O)		! staggered
C
C Room flags
C
	PARAMETER (RSEEN='100000'O)		! seen
	PARAMETER (RLIGHT='40000'O)		! lighted
	PARAMETER (RLAND='20000'O)		! land
	PARAMETER (RWATER='10000'O)		! water
	PARAMETER (RAIR='4000'O)		! air
	PARAMETER (RSACRD='2000'O)		! sacred (thief cant visit)
	PARAMETER (RFILL='1000'O)		! contains water
	PARAMETER (RMUNG='400'O)		! destroyed
	PARAMETER (RBUCK='200'O)		! part of bucket
	PARAMETER (RHOUSE='100'O)		! part of house
	PARAMETER (RNWALL='40'O)		! no walls
	PARAMETER (REND='20'O)			! part of endgame
C
C Room indices
C
...
C Object flags
C
	PARAMETER (VISIBT='100000'O)		! visible
	PARAMETER (READBT='40000'O)		! readable
	PARAMETER (TAKEBT='20000'O)		! takeable
	PARAMETER (DOORBT='10000'O)		! door
	PARAMETER (TRANBT='4000'O)		! transparent
	PARAMETER (FOODBT='2000'O)		! edible
	PARAMETER (NDSCBT='1000'O)		! don't describe
	PARAMETER (DRNKBT='400'O)		! drinkable
	PARAMETER (CONTBT='200'O)		! container
	PARAMETER (LITEBT='100'O)		! provides light
	PARAMETER (VICTBT='40'O)		! victim
	PARAMETER (BURNBT='20'O)		! burnable
	PARAMETER (FLAMBT='10'O)		! flaming
	PARAMETER (TOOLBT='4'O)			! tool
	PARAMETER (TURNBT='2'O)			! turnable
	PARAMETER (ONBT='1'O)			! turned on
	PARAMETER (FINDBT='100000'O)		! find
	PARAMETER (SLEPBT='40000'O)		! sleeping
	PARAMETER (SCRDBT='20000'O)		! sacred (thief wont steal)
	PARAMETER (TIEBT='10000'O)		! tieable
	PARAMETER (CLMBBT='4000'O)		! climbable
	PARAMETER (ACTRBT='2000'O)		! actor
	PARAMETER (WEAPBT='1000'O)		! weapon
	PARAMETER (FITEBT='400'O)		! fighting
	PARAMETER (VILLBT='200'O)		! villain
	PARAMETER (STAGBT='100'O)		! staggered
	PARAMETER (TRYBT='40'O)			! try to take
	PARAMETER (NOCHBT='20'O)		! don't check
	PARAMETER (OPENBT='10'O)		! open
	PARAMETER (TCHBT='4'O)			! touched
	PARAMETER (VEHBT='2'O)			! vehicle
	PARAMETER (SCHBT='1'O)			! searchable
C
C Object indices
C
...
C
	...
	PARAMETER (GWISH=263)			! global wish
	PARAMETER (GRWAL=265)			! global granite wall
	PARAMETER (GLOBAL=265)			! end of universals
...
C
	...
	COMMON /VOCAB/ BWORD(BWMAX),PWORD(PWMAX),PVOC(PWMAX),
	1	DWORD(DWMAX),DVOC(DWMAX),AWORD(AWMAX),AVOC(AVMAX),
	2	OWORD(OWMAX),OVOC(OVMAX),VWORD(VWMAX),VVOC(VVMAX)
C
C Game state
C
	LOGICAL TELFLG
