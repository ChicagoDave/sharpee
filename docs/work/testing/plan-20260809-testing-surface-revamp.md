...
- **Status**: CURRENT (started 2026-08-09, session d54d7e, under David's "implement it in the IDE" directive — the directive is the green light; the `packages/branch-tester` constraint is honored by REUSE, not modification: any change to branch-tester itself still stops for discussion).
...
- **Status**: DONE (2026-08-09, session d54d7e) Policies are `all-emitted-text | room-description | room-name-and-description` (no bare `room-name`). Next: gestures/pickers in cards.ts (source records must be kept by ordinal in main.ts and passed to compose), source-panel editor mode, the write bridge (`{write:…}` posts) + Swift writer/rename cascade, D6 picker, D9 editor rule, real-path tests. *Editor-mode approach (settled)*: `serializeAssertionTag(assertion)` IS exported from serializer — refactor compose around a shared `segmentPlan()` (per-turn assertion entries each carrying an optional DeleteRef: default/contains/notContains/state/event/channel/exact + ordinal + index, defaults deletion carrying the surviving fragments for `removeDefault`), from which `composeSegmentTranscript` builds the file and a new `composeSegmentLines` builds display lines (text via serializeAssertionTag, block lines, [SKIP]) with ✕ descriptors mapping straight onto SessionModel mutators. Swift passes `policy` (StoryHeaderAutoAssertion.read at open) and the project tests/ dir into the surface; writes land via `{write: {name, text, previousName?}}` posts → atomic write + child `continues:` line cascade (header-line rewrite before `---`). Hand-rename pinning deferred until the tab's rename affordance wires in (note: writer renames freely until then). Note for D8 strictness: an OPEN range's authored claims are session-memory only (the sidecar carries no assertions by D8's rule; closed segments' claims live in their `tests/` files once the writer lands) — reopen loses open-range claims; flag to David if it bites.
