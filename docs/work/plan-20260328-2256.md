...
**Created**: 2026-03-28
**Overall scope**: Implement the hybrid auto-compute + author-override action menu system described in ADR-136. The engine computes available actions each turn from the grammar × scope × traits intersection, applies annotation overrides, and sends an `actions` message through the bridge. Phases 4 and 5 from the ADR (template actions/popovers/keyboard shortcuts and reference client) are deferred. This plan covers the feature flag, core types, the computation engine, the bridge extension, and functional tests. Branch: `adr-136-context-actions`.
**Bounded contexts touched**: N/A — infrastructure/platform (engine, bridge, protocol); no domain behavior change
**Key domain language**: N/A
...
- **All platform changes** require the one-at-a-time discipline from `CLAUDE.md` — complete each file, show output, ask before proceeding.
- **Every command gets at least one functional test** — the `ActionMenuComputer` must have unit tests before the feature ships.

---

## Phases

### Phase 11: Build Flag + Core Types
- **Tier**: Small
- **Budget**: 100
- **Domain focus**: N/A — build system and type definitions
- **Entry state**: Branch `adr-136-context-actions` created; `build.sh` has flag parsing at lines 134-194; no `ContextAction` type exists anywhere in the codebase; `INCLUDE_CONTEXT_ACTIONS` is not defined
- **Deliverable**:
  - `build.sh` updated: `--include-context-actions` flag parsed; esbuild `--define:globalThis.INCLUDE_CONTEXT_ACTIONS=true` injected when flag present, `false` when absent (same pattern as `PARSER_DEBUG` and `DEBUG_PRONOUNS`)
  - `ContextAction` interface added to `packages/if-domain/src/` (or `packages/core/src/` — whichever holds shared protocol types; confirm during implementation)
  - `ActionsMessage` interface added to `packages/bridge/src/protocol.ts` alongside the existing outbound message union
  - `actionMenu?: boolean` and `maxActions?: number` fields added to `ClientCapabilities` in the bridge protocol
  - `action-hint` and `action-suppress` annotation kinds documented as string constants (no infrastructure change required per ADR — annotation system already supports arbitrary kinds)
  - `./build.sh -s dungeo` passes (no regression)
  - `./build.sh -s dungeo --include-context-actions` passes (new flag accepted without error)
- **Exit state**: Types exist and are importable; build flag gates compilation; no runtime behavior changed yet
- **Status**: PENDING

### Phase 12: ActionMenuComputer Module
- **Tier**: Medium
- **Budget**: 250
- **Domain focus**: N/A — engine computation module
- **Entry state**: Phase 11 complete; `ContextAction`, `ActionsMessage`, and `ClientCapabilities.actionMenu` types exist; `GrammarEngine.getRules()` and `GrammarEngine.getRulesForAction()` available in `packages/if-domain/src/grammar/grammar-engine.ts`; scope methods `getVisibleEntities()`, `getTouchableEntities()`, `getCarriedEntities()` available on `WorldModel`
- **Deliverable**:
  - New module `packages/engine/src/action-menu-computer.ts` (or `packages/stdlib/src/action-menu/` — confirm placement during implementation based on import structure)
  - `ActionMenuComputer` class with a single public method: `compute(world, actorId, grammarEngine): ContextAction[]`
  - Computation logic:
    - Collects entities by scope level (carried, reachable/touchable, visible)
    - Iterates `grammarEngine.getRules()`; for each rule with a `:target` slot, checks `traitFilters` against entity traits to decide inclusion
    - Extracts verb label from pattern string (e.g. `"take :target"` → `"Take"`)
    - Produces direction actions from the current room's `RoomTrait` exits
    - Produces intransitive actions (look, inventory, wait) as always-available entries
    - Deduplicates by `(actionId, targetId)` pair — same action+target from alias verbs collapses to one entry, keeping the shorter/primary verb
    - Applies `action-hint` annotations: merges/overrides auto-computed entries or adds new ones; respects `condition` to filter by current trait state
    - Applies `action-suppress` annotations: removes matching entries by `actionId`
    - Categorizes each entry (`movement`, `interaction`, `inventory`, `communication`, `combat`, `meta`, `story`) based on action ID prefix or explicit override
    - Sorts by category then priority (descending) then alphabetically
    - Caps at `maxActions` (default 40) and per-entity max (default 8)
  - Entire module body wrapped in `if (globalThis.INCLUDE_CONTEXT_ACTIONS)` guard so tree-shaker eliminates it when flag is false
  - Unit tests in `packages/engine/tests/action-menu-computer.test.ts` (or co-located with the module):
    - Given a room with a lantern (portable) and a trapdoor (openable), `compute()` returns entries for "take lantern", "examine lantern", "open trapdoor", "examine trapdoor", plus direction actions and intransitives
    - Alias deduplication: "take" and "get" for the same entity produce one entry
    - `action-suppress` removes a matching entry
    - `action-hint` with a false condition does not appear; with a true condition does appear and overrides priority
    - Cap enforcement: with 50 entities in scope, total actions capped at 40
  - `pnpm --filter '@sharpee/engine' test` passes (or whichever package hosts the module)
- **Exit state**: `ActionMenuComputer` is a standalone, tested module; it is not yet wired into the turn cycle
- **Status**: PENDING

### Phase 13: Engine Turn Cycle Integration + Bridge Flush
- **Tier**: Small
- **Budget**: 100
- **Domain focus**: N/A — engine wiring and bridge protocol
- **Entry state**: Phase 12 complete; `ActionMenuComputer` module exists and is tested; bridge `protocol.ts` has `ActionsMessage` type; `executeTurn()` in `packages/engine/src/game-engine.ts` has a known hook point after text processing (~line 728)
- **Deliverable**:
  - `game-engine.ts` updated: after existing text/status processing, check `ClientCapabilities.actionMenu`; if true and `INCLUDE_CONTEXT_ACTIONS` is defined, call `ActionMenuComputer.compute()` and attach the result to the turn output
  - Bridge `flushTurn()` updated: flush order becomes blocks → events → status → actions; only sends `ActionsMessage` if the result array is non-empty
  - Integration test: a transcript test or inline test that enables `actionMenu: true`, runs a turn in the Living Room scene, and asserts that the output contains an `actions` message with at least one movement action and one interaction action
  - `./build.sh -s dungeo` passes (no regression — flag absent means actions are not computed)
  - `./build.sh -s dungeo --include-context-actions` passes
  - Unit transcript tests (`stories/dungeo/tests/transcripts/*.transcript`) pass
  - Full walkthrough chain passes
- **Exit state**: When `--include-context-actions` is used and `ClientCapabilities.actionMenu` is true, every turn produces a valid `actions` message through the bridge; text-only clients are completely unaffected
- **Status**: PENDING
