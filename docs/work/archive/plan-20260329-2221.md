...
# Session Plan: ADR-097 Migration — Dungeo Legacy Event Pattern Removal

**Created**: 2026-03-29
**Overall scope**: Migrate all 47 Dungeo story files from the legacy `action.success`/`action.blocked` event pattern to ADR-097 semantic domain events (event type is a domain name; messageId is the full qualified key embedded in event data). Once all story code is migrated, delete the legacy `handleActionSuccess()` and `handleActionFailure()` handlers and their routing cases from the text service.
**Bounded contexts touched**: stories/dungeo (story actions, handlers, capability behaviors); packages/text-service (platform cleanup after story migration is complete)
**Key domain language**: N/A — mechanical alignment to existing ADR; no domain behavior change

## Migration Pattern Reference

Legacy (blocked phase):
```typescript
...
For capability behaviors using `createEffect`:
```typescript
// blocked: replace createEffect('action.blocked', ...) with createEffect('dungeo.event.basket_lower_blocked', ...)
// report:  replace createEffect('action.success', ...) with createEffect('dungeo.event.basket_lowered', ...)
```
...
### Phase 11: Migrate 38 Blocked-Only Story Actions (Category 1 — Mechanical)
- **Tier**: Medium
- **Budget**: 250
- **Domain focus**: Dungeo story actions — blocked phase only; no report phase changes
- **Entry state**: All 38 actions listed in the audit emit `context.event('action.blocked', { actionId, messageId, reason })` in their `blocked()` phase; text service still has `handleActionFailure()` routing these
- **Files to change** (38 total):
  `inflate`, `deflate`, `set-dial`, `say`, `press-button`, `turn-switch`, `turn-bolt`, `launch`, `lift`, `lower`, `incant`, `send`, `untie`, `push-dial-button`, `push-panel`, `ring`, `tie`, `wave`, `wind`, `break`, `burn`, `melt`, `dig`, `fill`, `light`, `walk-through`, `diagnose`, `grue-death`, `falls-death`, `commanding`, `answer`, `puzzle-look`, `raise-basket`, `lower-basket`, `objects`, `room`, `rname`, `push-wall` (blocked phase only)
- **Deliverable**:
  - All 38 `blocked()` methods updated: `'action.blocked'` event type replaced with `'dungeo.event.{action_name}_blocked'`; `actionId` field removed; messageId becomes the fully-qualified key `${ACTION_ID}.${result.error || DEFAULT_MSG}`; `reason` field removed
  - `push-wall` blocked phase migrated here; its report phase migrated in Phase 12
  - Walkthrough chain (wt-01 through wt-16) passes after all 38 changes
- **Exit state**: No `'action.blocked'` event emissions remain in any Dungeo story action file; `handleActionFailure()` is still present in the text service but only serves the remaining 2 capability behaviors
- **Status**: CURRENT

### Phase 12: Migrate 8 Report-Phase Actions and 2 Handlers (Category 2 — Domain Named)
- **Tier**: Medium
- **Budget**: 250
- **Domain focus**: Dungeo story actions and handlers that emit `action.success` in their report phase — each needs a semantically meaningful domain event type name
- **Files to change** (10 total):
  - `actions/talk-to-troll/talk-to-troll-action.ts` — report: `dungeo.event.talked_to_troll`
  - `actions/push-wall/push-wall-action.ts` — report: `dungeo.event.pushed_wall`
  - `actions/pour/pour-action.ts` — report: 6 variants, all `dungeo.event.poured` with different messageIds
  - `actions/puzzle-take-card-blocked/puzzle-take-card-blocked-action.ts` — report: `dungeo.event.card_examined`
  - `actions/gdt/gdt-action.ts` — report: `dungeo.event.gdt`
  - `actions/gdt/gdt-command-action.ts` — report: `dungeo.event.gdt_command`
  - `handlers/balloon-handler.ts` — balloonExitAction report and blocked: `dungeo.event.balloon_exit` / `dungeo.event.balloon_exit_blocked`
  - `handlers/royal-puzzle/puzzle-handler.ts` — 2 x `action.success` with `message` fallback: migrate to `dungeo.event.puzzle_move`; if the `message` field (not messageId) is in use, register the string in lang-en-us under a proper key first
- **Deliverable**:
  - All `action.success` emissions in report phases replaced with domain event types
  - Balloon handler remaining `action.blocked` also migrated
  - Puzzle handler fallback path resolved: either a messageId is registered in lang-en-us, or the event becomes `game.message` (which has an existing routing path)
  - Walkthrough chain passes after all changes
- **Exit state**: No `'action.success'` or `'action.blocked'` event emissions remain in any Dungeo story action or handler file
- **Status**: PENDING

### Phase 13: Migrate 2 Capability Behaviors (Category 3 — createEffect Pattern)
- **Tier**: Small
- **Budget**: 100
- **Domain focus**: Dungeo capability behaviors using `createEffect('action.success/blocked', ...)`
- **Files to change** (2 total):
  - `traits/basket-elevator-behaviors.ts` — 4 `createEffect` calls (lower report, raise report, lower blocked, raise blocked)
  - `traits/egg-behaviors.ts` — 2 `createEffect` calls (opening report, opening blocked)
- **Deliverable**:
  - `basket-elevator-behaviors.ts`: `createEffect('action.success', ...)` → `createEffect('dungeo.event.basket_lowered', ...)` and `createEffect('dungeo.event.basket_raised', ...)`; `createEffect('action.blocked', ...)` → `createEffect('dungeo.event.basket_lower_blocked', ...)` and `createEffect('dungeo.event.basket_raise_blocked', ...)`
  - `egg-behaviors.ts`: `createEffect('action.success', ...)` → `createEffect('dungeo.event.egg_opened', ...)`; `createEffect('action.blocked', ...)` → `createEffect('dungeo.event.egg_open_blocked', ...)`
  - Walkthrough chain passes (basket and egg puzzles exercised)
- **Exit state**: No `createEffect('action.success', ...)` or `createEffect('action.blocked', ...)` calls remain in any Dungeo trait or behavior file
- **Status**: PENDING

### Phase 14: Remove Legacy Handlers from Text Service (Platform Cleanup)
- **Tier**: Small
- **Budget**: 100
- **Domain focus**: N/A — platform structural cleanup; requires David's approval before starting (CLAUDE.md: platform changes require discussion first)
- **Entry state**: Phases 11–13 complete; no `action.success` or `action.blocked` events emitted anywhere in Dungeo story code; `handleActionSuccess()` and `handleActionFailure()` have no live callers
- **Deliverable**:
  - `packages/text-service/src/handlers/action.ts`: delete `handleActionSuccess()`, `handleActionFailure()`, `ActionSuccessData`, `ActionFailureData`; delete file if nothing remains
  - `packages/text-service/src/text-service.ts`: remove `case 'action.success':` and `case 'action.failure': case 'action.blocked':` routing; remove `if (event.type.startsWith('action.')) { return null; }` guard in `tryProcessDomainEventMessage()`; remove corresponding imports
  - `packages/text-service/src/handlers/index.ts`: remove re-exports for deleted functions
  - `pnpm --filter '@sharpee/text-service' test` passes
  - Full walkthrough chain passes
- **Exit state**: Text service has a single routing path (domain events with embedded messageId); `action.*` routing is gone; ADR-097 is fully enforced
- **Status**: PENDING
