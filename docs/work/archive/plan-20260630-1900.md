...
- **Exit state**:
  ...
  - Unit and transcript tests pass.
  - `./repokit build dungeo` still green (no pipeline regression).
  - `v2_adr201_p1` merged to `main`.
...
- **Deliverable**:
  ...
  **6. Write the structural reconciliation pass** (ADR-201 §3.2 — four steps in order):
  ...
  Step 2 — Punctuation merging: when a run ownsLeadingPunct (',') is adjacent to a quoteOpen run,
             emit the comma before the quote (`, "`); when quoteClose is adjacent to a run
             ownsTrailingPunct, place the period inside (`."`). Never double or orphan marks.
...
- **ACs satisfied**: ADR-201 AC-1 (parser half: `{capitalize pronoun:subject}` legal), ADR-201
  AC-2 (parser half: `{quote:utterance}` parses), ADR-201 AC-10 (all three rejection cases:
  unbound param, unknown head, `{capitalize numeral:…}`).
