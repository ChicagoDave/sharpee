...
> is still PENDING and still blocked on an ADR-328 plan that does not yet exist. Resume it here when
> ADR-328's D2 execution entry lands.
...
- **Status**: DONE (2026-08-26) — chord edits landed (session e822b1): grammar, analyzer, IR, EBNF, 4.0.0 bump, 50-file fixture migration, `tests/adr-327-phase1.test.ts`; `pnpm --filter '@sharpee/chord' run test:ci` green — 69 files, 1039 passing (verified twice, independently, by the session-checkpoint agent at 06:38:13Z after the last edit at 06:36:43Z). Two shape gaps found and resolved (placement/head collision; `define condition` as a second carrier scope) — see `phase-1-grammar-shape.md` Landing notes.
...
- **Status**: DONE (2026-08-27, session a3a4af) — **paper trail half only; the reference-surface half moved to its own plan (see below).**
  **Flips landed**: ADR-325 D3h's two examples amended in place — the `create the player` block to the D10 shape (`create Jack` / `a person` / `playable` + a `before the game starts` block), and `when the player moves, while it is approaching` to `while the wandering mercenaries is approaching` with `move`/`change` naming them outright, matched to the landed corpus at `branch-stories/secret-letter/mercenaries.chord:173` rather than the plan's shorthand `the mercenaries`; `restart waiting` kept bare (ADR-325 D3c's owner-first timer form, which ADR-327 D2 leaves alone). ADR-264 D2's `raise its suspicion by 5` clause struck, and — beyond ADR-327's named scope, because striking D2 alone left the ADR self-contradictory — the same dead spelling struck from the D4/`set` target list and from D3's conditions, under one dated amendment note. ADR-328 D7 and its acceptance item 3 needed nothing: already flipped in session 8ae644.
  **`chord-grammar-changes.md` finalized**: the ADR-327 D9/D10 row's Decision cell said "chord half landed"; the Phase 4 corpus cutover and its gate numbers are now recorded on it.
  **Paper trail verified**: `packages/chord/src/version.ts` carries the complete 4.0.0 / ADR-327 entry (D1–D4, D6, D8, D9, D10, `story language 3`→`4`); `tests/language-version.test.ts` EBNF pin green.
  **Closing regression gate, all green** (working tree was four `.md` files — no source moved): repo `tsc --noEmit` clean; chord 1064, story-loader 637, stdlib 1637, engine 637, world-model 1492, bootstrap 52, devkit 172, world-index 169, character 567, platform-browser **143** (Phase 4 recorded 138 — five tests added since, not a regression); Dungeo walkthrough chain **952 passing across 17 transcripts**, the baseline unmoved.
  **Scope finding — the reference surface is not what the plan measured.** The book is a no-op (`docs/book/` is the TypeScript authoring book: zero `chord` fences), and `docs/reference` does not exist. sharpee.net's Chord reference is in-repo at `website/src/app/**/content.mdx` — and it carries **187 removed-spelling sites across 83 files**, not 10: 86 `on`/`after <gerund> it` heads, 51 `create the player`, 27 `while`/`when it is` conditions, 22 `change it to`/`move it`, 1 possessive `its`. The plan's "10 doc files measured" was a broad grep catching English prose. Worse, the reference documents **none** of what ADR-327 landed — zero occurrences of `playable`, `before the game starts`, `change the player to`, or an actor-explicit head anywhere in it. That makes the remaining work a spelling sweep **plus net-new documentation of four constructs**, feeding the IDE's shipped docs-tab, which is documentation authoring rather than the tail of a code landing. Split out by owner ruling (David, 2026-08-27) — see `docs/work/` for its own plan. Acceptance item 4's ADR/EBNF/grammar-changes half is satisfied here; its ADR-272 reference half is owed by that plan.
...
**Corrected 2026-08-27 (session a3a4af):** the paragraph below was written before the pointer moved and had gone stale — `docs/context/.current-plan` resolves to **this** plan, verified by reading the pointer directly. It was reported as stale by `pre-session-audit` before being fixed here.

> *(Superseded, kept for the record.)* Per DEVARCH rule 18b, `docs/context/.current-plan` currently names `docs/work/backlog-tier1-2-platform/plan.md`, which still has live PENDING phases (4–8, the parser-scope-and-story-grammar seam). This plan does **not** repoint `.current-plan` — that requires David to first dispose of the backlog plan (done-but-unmarked / still-live / abandoned), which has not been asked. `docs/work/adr-327-explicit-references/plan.md` exists as a durable plan file but is not yet the active pointer target.
