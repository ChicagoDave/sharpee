...
- **Status**: DONE — completed 2026-07-30 (session 405116). Both exit clauses met:
  `pnpm --filter '@sharpee/repokit' test consumer-gen` passes (17 tests, up from 11), and
  `./repokit test:npm stories/dungeo --local` installs and runs the suite **115 passing,
  0 failures**. Closing it needed three defects cleared, not one — the `pack()` shape fix
  (2026-07-29), then #201, then a stale staging tree. See the 2026-07-30 session summary.
  The bare-name invocation this phase's exit text names is #202, out of scope.
- **Status history**: BLOCKED — the `pack()` half was delivered and verified (2026-07-29, session 5c4586); the "and installs" half of the exit state was blocked on a newly-found independent defect.
