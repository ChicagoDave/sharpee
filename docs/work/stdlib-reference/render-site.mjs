#!/usr/bin/env node
/**
 * render-site.mjs — render docs/reference/stdlib-reference.md into
 * site/stdlib-reference.html, in the site's hand-written HTML conventions.
 *
 * Adapted from docs/work/chord-language-reference/render-site.mjs (the proven
 * chord-reference renderer) with one semantic change: this document uses
 * blockquotes as real content ("platform note" callouts), so blockquotes are
 * rendered as <blockquote> — only the internal `> **Status:` banner is
 * stripped from the published page.
 *
 * Public interface: `node render-site.mjs` — writes site/stdlib-reference.html
 * and prints the byte count. Owner context: docs tooling (stdlib-reference
...
const outPath = path.join(repoRoot, 'site', 'stdlib-reference.html');
...
<head>
  ...
  <title>Standard Library Reference — Sharpee</title>
  <meta name="description" content="The author reference for Sharpee's standard library: every standard action, trait, and runtime service, framed through the Chord .story surface.">
