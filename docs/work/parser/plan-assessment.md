...
```typescript
grammar.define('polish :target').hasTrait(TraitType.PORTABLE).matching({ material: 'metal' }); // ad-hoc property check
```
...
const eatAction: Action = {
  id: 'if.action.eating',
  preconditions: [{ scope: ScopeLevel.CARRIED, implicitAction: 'if.action.taking' }],
  // ...
};
```
...
RESPONSE: I like preconditions, but need more scenarios to validate the change.
...
RESPONSE: Think in terms of events. We should handle scope failure in blocked() in our 4-phase action handers.
...
RESPONSE: This is nitpicky. The parser and stdlib are highly coupled and that's just the nature of an IF engine. We could go back and forth on the "correct boundary" and never make progress.
...
RESPONSE: This may be a part of this refactor or a separate ADR.
...
RESPONSE: We just implemented this in a recent ADR.
