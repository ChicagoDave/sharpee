...
```bash
# Build your story, then run all its transcript tests
sharpee build --test

# Run transcripts without rebuilding
sharpee test

# Specific transcripts, chained (game state persists between files)
sharpee test --chain walkthroughs/wt-01.transcript walkthroughs/wt-02.transcript

# With verbose output
sharpee build --test --verbose

# Stop on first failure
sharpee build --test --stop-on-failure
```

`sharpee build --test` compiles your story and immediately runs its transcript tests in one step (no separate test bundle). The test runner finds:
...
- Run as a chain with `node {story}-test.js --test --chain walkthroughs/wt-*.transcript`
...
| Flag | Description |
|------|-------------|
| `--test` | Compile the story, then run its transcript tests |
...
| Flag | Description |
|------|-------------|
| `[files…]` | Specific transcript files (default: everything under `tests/`) |
