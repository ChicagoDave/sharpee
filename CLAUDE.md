...
<!-- devarch:start -->
<!--
  DevArch Agent Lifecycle Rules — Base (v1.3.3)
...
10. **Every command gets at least one functional test.** The test invokes the command and asserts on the resulting state change or rejection. No command ships without a test that proves it works.
    - Functional tests verify a single unit of behavior: given input X, assert output/state Y.
    - Test the mutation, not the mock. If the command writes to a database, the test asserts on what was persisted. If it emits an event, the test asserts on the event payload.

11. **Every scenario from planning gets at least one behavioral test.** If the session plan describes a user workflow or business rule, there must be a test that exercises that path end-to-end.
    - Behavioral tests express business language: "when a customer places an order with insufficient inventory, the order is rejected and an `OrderRejected` event is emitted."
    - These tests are the living documentation of what the system does. They survive refactors because they test outcomes, not implementation.

12. **Do not commit code without running the existing test suite.** If tests fail, fix them before proceeding. If a test is genuinely obsolete due to an intentional design change, update or remove it — do not skip it.
...
13. After you **Edit or Write a file containing side-effect functions** (names containing: execute, handle, process, save, update, delete, remove, create, send, dispatch, publish, persist, submit, store), run the `mutation-verification` agent on the changed files.
...
14. During **long sessions with significant work**, run the `session-checkpoint` agent roughly every 60 minutes of active development.
...
15. If a PostToolUse hook emits a **`[Budget ...]`** message, acknowledge it briefly and adjust behavior:
...
16. **Session-end signals** — ANY of the following trigger the `work-summary-writer` prerequisite:
...
17. After the work summary is written:
