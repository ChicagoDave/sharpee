...
# ── Fernhill browser client — gitignored, so (re)build it on deploy. ──
# /play embeds public/web/fernhill/index.html in an iframe. That tree is
# generated by repokit's --browser mirror (mirrorToWebsite) and ignored by
# .gitignore:152, so git pull can never supply it: without this step a fresh
# checkout serves /play with an empty iframe. Guarded like the playground step
# above — a missing game is not a reason to leave sharpee.net unbuilt.
#
# This regressed once already. ADR-302 D16 moved the story from stories/ to
# branch-stories/ (18d65ab3, 2026-08-05); repokit's resolver knew only
# stories/|tutorials/, so the mirror stopped running and nobody noticed for
# nine days — the artifact is untracked, so no test and no git status ever
# mentioned it. (plover, 2026-08-14.)
log "Rebuilding the Fernhill browser client (./repokit build fernhill --browser) ..."
if ( cd "$REPO_ROOT" && ./repokit build fernhill --browser ); then
  log "Fernhill browser client refreshed."
else
  warn "fernhill browser build failed — /play will show an empty iframe."
fi

# Assert the ARTIFACT, not the exit code. /play embeds this exact path, and a
# build that reports success while mirroring nothing looks identical to one
# that worked — which is precisely how the nine-day outage above stayed
# invisible. Check the file the page actually requests.
