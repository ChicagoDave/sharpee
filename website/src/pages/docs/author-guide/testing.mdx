...
```bash
# Build your story with a test runner bundle
npx sharpee build --test

# Run all transcript tests
node my-story-test.js --test tests/transcripts/*.transcript

# Run walkthroughs as a chain
node my-story-test.js --test --chain walkthroughs/wt-*.transcript

# With verbose output
node my-story-test.js --test --verbose walkthroughs/wt-*.transcript

# Stop on first failure
node my-story-test.js --test --stop-on-failure walkthroughs/wt-*.transcript
```

`npx sharpee build --test` compiles your story and produces a `{story}-test.js` bundle — a single file that includes the Sharpee engine and your story, ready to run transcript tests. The test runner finds:
