...
lowercase, which is how a requirement stays distinct from prose — a
name (`the player`, `the secateurs`) anywhere, or `it`/`its` for the
carrier inside a `define trait` body, as here. One fixture exercises
every form:

```chord
define trait stockable
  on the player stocking
...
tests membership in an open condition, plain `must be` compares
...
```chord
  on the player sharpening
    the secateurs must be dull: already-sharp
    refuse when the player wears the gardening mittens: mittens-on
    change the secateurs to sharp
    phrase sharpened
  end on
```
