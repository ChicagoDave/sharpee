...
owner: `on the player opening`, where `it` means this entity for the rest of the
...
An entity-owned `every turn` clause fires wherever its owner is, whether
or not the player is there: the scarecrow performs either way, and what
it says is tagged with the owner's location and the player's presence so
the client decides what to show — by default it hides narration tagged
absent. A `, once` or a dice roll is consumed off-stage all the same.
Note `while dusk`: a story-level state is referenced as a
