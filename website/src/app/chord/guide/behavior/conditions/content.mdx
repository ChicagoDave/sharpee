...
`is in` also takes a region: `the barrow is in the Grounds` is a
membership test — true in any member room, through nested regions — and
needs no `landing` line. A [timer](/chord/guide/flow/timers) reads two
ways: `is` names its current turn (`while search is lingering`) and
`has` reads its lifecycle (`has started`, `has not started`, `has
expired`; `is expired` is rejected). Under `use chapters`,
`during <name>`, `before <name>`, and `after <name>` test the current
[chapter](/chord/guide/flow/chapters).

State words resolve against their subject: `when the cat is dozing` works
