...
spot, in a room's description or any other entity's: the plaque below
could carry `{weather}` too, and `examine plaque` would resolve it
exactly as `look` does here. Every bare lowercase marker must name something the story
declares: a phrase or a text hatch, or — inside a phrase spoken `with
<name> = <value>` — one of the phrase's own parameters. A parameter
that names an entity takes a leading article hint: `{the item}`,
`{a item}` / `{an item}`, `{some item}`, and `{bare item}` for no
article at all, when the prose supplies its own determiner
(`another {bare item}` renders "another pear"). `{capitalize the item}`
starts a sentence with it. Here `weather` is a
