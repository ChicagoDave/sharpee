...
exactly one implementation, an `on the player cutting` clause, its own or
from a composed trait. The story fails to load with none, or with
two. The same rule binds `diggable` to `on the player digging`.
...
create the straw bale
  ...
  on the player cutting
    change the straw bale to loose
    phrase twine-cut
  end on
```
