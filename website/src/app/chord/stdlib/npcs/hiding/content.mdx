...
Two spots, two seams: the hamper's `on the player hiding` guard refuses with its
...
Interceptors: the hiding spot's `on the player hiding` clauses are consulted
(the hamper above); revealing has no interceptor surface (flagged as a
minor asymmetry).

Three more honest flags remain. The design puts every action outside a quiet
