**talk** (`talking`) covers the verbs `talk to/with X`,
...
**ask** (`asking`) and **tell** (`telling`) take
...
the person's `on the player asking` / `on the player telling` clause is the catch-all,
...
alike, and the `on the player asking` catch-all speaking only on a miss.
...
Interceptors: `on the player talking` on the person is talk's override seam.
