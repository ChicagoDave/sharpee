...
`on the player attacking` clause fires; a weapon *inferred* from inventory is
...
- **A combatant** (the `combatant` trait) refuses
  ...
  Real combat *is* the interceptor (normally the basic-combat
  extension), and since ADR-215 a `.story` registers it itself: `use combat`
  ...
  damages through the `health` trait, and hands stdlib the result
  to narrate. Kills emit death events; a dead target refuses
  `already_dead`.
...
| Success | combat narration overridable via the `attacking-combat-*` alias family (the extension's, carried by the lang layer) · `target_broke` · `target_damaged` / `target_destroyed` |
| Events | `attacked` · death/knockout and exit-revealed events after the blow text |

Interceptors: `on the player attacking` on the target (a story wanting scripted
...
interceptor on the same trait seam.
