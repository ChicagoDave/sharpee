**switch on** (`switching_on`) takes the verbs `turn on X`,
...
**switch off** (`switching_off`) takes the verbs `turn off X`,
...
create the safelight
  ...
  phrase detail while the safelight is lit:
...
picks up its `detail while the safelight is lit` sentence, the enlarger's `on
the player switching_on` guard refuses with its own phrase, and the `starts on`
...
for story reactions. The interceptors are `on the player switching_on` / `on
the player switching_off` on the device, as with the enlarger above.
