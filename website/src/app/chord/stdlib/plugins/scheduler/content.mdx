...
by a condition and optionally once. A **fuse** counts down over a set
number of turns and then fires. It supports an optional per-turn tick
condition (turns can refuse to count), repeat, cancellation with cleanup,
entity binding (auto-cleanup when the entity goes away), and mid-flight
adjustment. Fuses are a
...
A `define sequence` (chord-language.md) compiles to **one daemon**
...
An `on every turn [while …][, once]` clause (chord-language.md)
...
header's own body it is the story's clause, one daemon with **no gate**:
a background clock that ticks wherever the player is (weather, off-stage
simulation; ADR-236).
