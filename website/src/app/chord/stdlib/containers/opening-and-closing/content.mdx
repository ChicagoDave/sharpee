**open** (`if.action.opening`) parses the verbs `open`, `open up`,
...
**close** (`if.action.closing`) parses the verbs `close`, `shut`, and
...
refusal, and the story-wide `define phrase` rewrites every plain close.
