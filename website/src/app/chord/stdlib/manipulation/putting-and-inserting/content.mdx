...
`put_in` re-voices both phrasings, and the bin's single `on the player putting`
...
Interceptors: `on the player putting` / `on the player inserting` on the item or the
container. An INSERT consults `on the player inserting` first, then (through the
delegation) `on the player putting` on both entities, so register a given entity
under **one** of the two gerunds, not both, or it will run twice;
`on the player putting` covers both phrasings, and in `put all in the case` a
