**remove X from Y** (`removing`) takes an item out of a
...
Interceptors: the item slot consults `on the player removing` and then `on the player taking`, in that order; the source consults `on the player removing` only, and so does
