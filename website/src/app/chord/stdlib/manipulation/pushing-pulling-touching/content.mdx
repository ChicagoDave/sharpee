**push** (`if.action.pushing`) covers the verbs `push`, `press`, `shove`,
...
story reacts to `if.event.pushed` (it carries the direction) or an
`after pushing it` clause to actually change the world; which wall opens
...
**pull** (`if.action.pulling`) covers the verbs `pull`, `drag`, `yank`,
...
lever opens the sluice) is story logic reacting to `if.event.pulled` or
written as an `on pulling it` / `after pulling it` clause.

**touch** (`if.action.touching`) covers the verbs `touch`, `feel`, `rub`,
