...
modern replacement for `reactions` is an `on the player showing` clause on the
...
vetoes the trade, the ferryman's `after the player giving` reacts once it commits,
and his `on the player showing` replaces the flat `shown` line entirely. For a
...
Interceptors: `on the player giving` / `after the player giving` and `on the player showing` /
`after the player showing`, on the item or on the recipient/viewer.
