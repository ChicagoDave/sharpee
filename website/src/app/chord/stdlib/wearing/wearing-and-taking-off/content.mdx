...
out from underneath (the layer-default quirk, live), and the torc's `on the player taking_off` guard is a cursed garment in one clause.
...
Interceptors: `on the player wearing` / `on the player taking_off` on the garment: a cloak
