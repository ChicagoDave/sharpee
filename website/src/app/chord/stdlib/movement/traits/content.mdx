...
`on`/`after` clauses, including `after the player entering`, the room-arrival
...
(additive: regions nest by containing each other), `after the player entering`
/ `after the player leaving` react to boundary crossings (`leaving` exists only
