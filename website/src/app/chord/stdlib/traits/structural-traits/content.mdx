...
- `unlisted` on a container or supporter keeps the room from listing
  what it holds ("On the rope wares you see a length of rope." is never
  printed) — for a holder whose own prose describes its contents. The
  contents stay in scope: examinable, takeable, reachable by clauses.
...
lists members (additive across lines), `after the player entering` / `after
the player leaving` react to boundary crossings (`leaving` exists only on region
...
by an always-on engine plugin, emitting `scene_began` /
...
never touches it directly; the header's `title:`, `authors:`,
`story-version:` and `description:` flow through and the engine
materializes the trait.
