**look** (`looking`) parses bare `look`, `l`, and `look
around`. It rerenders the room: name and description (a first visit
...
**examine** (`examining`) parses `examine X`, `x X`, `inspect
X`, `check`/`view`/`observe X` (ADR-230 D4), `look at X`, and `look
...
create the brass telescope
  ...
  phrase detail while the brass telescope is on:
...
Interceptors: `on the player examining` / `after the player examining` on the target
...
on; use the room's `after the player entering` or an `on every turn` daemon
instead.
