...
**Chord**, Sharpee's story language. It unfolds over one winter night, with
one player, thirteen rooms, three NPCs, a boiler with opinions, and the deed
that keeps Fernhill in the family, if you can find it before dawn.
...
rule scoped to a moment (`after the player entering while the player has
the deed`). The world holds the logic; the prose stays prose.

The rule of thumb, when an idea resists plain expression, runs like this:
**IF concepts belong in Chord; engine concepts belong in Sharpee; general
computation belongs in TypeScript.** If Chord can't say an interactive-fiction idea, that's a gap
...
|---------|--------------|------------|
| [The world](/learn/fernhill/world) | A — rooms, doors, regions | compass map, `through` doors, darkness, live blocked exits, crossing reactions, daemons |
| [Things](/learn/fernhill/things) | B — objects & tools | containers, the declarative capability family, concealment, readables, scenery craft |
| [People](/learn/fernhill/people) | C — NPCs & conversation | guard, patrol, give-gating, topic tables |
| [The long night](/learn/fernhill/time) | D — time & causality | timelines, the fuse, recurring events, state machines |
| [State & verbs](/learn/fernhill/state) | E — custom mechanics | entity state chains, authored traits, custom verbs, refusals |
| [Endings & text](/learn/fernhill/endings) | F, G1–G2 — payoff | owner-attached scores, three endings, phrase strategies, gated detail |
| [The browser](/learn/fernhill/browser) | G4–G5 — presentation | sound, images, ambient beds, dynamic channels, graceful text degradation |
...
in `fernhill.story`. A pure Chord story needs no TypeScript at all; the only
optional wiring you'll meet in the last chapter is a custom `browser/index.html`
that names where a channel's output lands. The working loop is three commands:

```shell
sharpee build            # compile gate — every load error, before anything runs
sharpee test             # replay your test tree
...
The full grammar reference lives in the
[Chord grammar reference](/chord/reference/grammar); each chapter names the
sections it draws on. Start with [the world](/learn/fernhill/world).
