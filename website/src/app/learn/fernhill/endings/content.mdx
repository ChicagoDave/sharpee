...
(`award softened, once when the boiler is softened`), or the boiler's own
switch-on clause. The `, once` modifier keeps repeats honest.

## Three endings, all real

**Win** comes from walking out the gates carrying the deed:
...
**Lose** comes from the timeline's `at turn 130 → lose dawn-comes`
(*The long night*).

**Die** comes from the fuse's `kill the player fuse-blast when the fuse
is lit`.

`win` / `lose` / `kill the player` each name their ending phrase. Write
all your endings for real and test all of them; Fernhill's test tree
drives each ending as its own branch, including the death.
...
`sticky`). When your tests claim strategy lines, assert structurally
...
Before the diary is read, the sentence reads clean, with no seam. After it
is read, the same room points you at the photograph. The photograph itself gains a `phrase
