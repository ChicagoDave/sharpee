/**
 * Fixture RandomService for dungeo's in-src unit tests (ADR-293 D6
 * test-fixture exemption — `createSeededRandom` is callable from test
 * fixtures only).
 *
 * Mirrors the engine implementation's per-point derivation without importing
 * engine. Force lookup is a pass-through, like the engine's until Phase C.
 *
 * Public interface: `createFixtureRandomService(masterSeed?)`.
 * Owner context: stories/dungeo test harness.
 */
