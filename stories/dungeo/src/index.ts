...
export class DungeoStory implements Story {
  ...
  extendLanguage(language: LanguageProvider): void {
    ...
    // Egg - only thief can open (ADR-090 universal dispatch)
    ...
    // Room info messages (OBJECTS command - no visible objects)
  ...
  onEngineReady(engine: GameEngine): void {
    ...
    if (scheduler) {
      ...
      // Register Troll Recovery daemon (wake up after knockout)
      registerTrollRecoveryDaemon(scheduler, this.undergroundIds.trollRoom);
    ...
    if (npcService) {
      ...
      // Register troll NPC behavior (entity created in underground.ts)
