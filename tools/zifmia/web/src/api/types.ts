...
/**
 * Successful body of `POST /admin/identities/:id/passcode_reset`.
 *
 * The `passcode` plaintext crosses the wire EXACTLY ONCE — the
 * client must display it for the admin to copy and then release the
 * reference so it does not linger in DOM or memory beyond the dialog.
 */
export interface PasscodeResetResponse {
  passcode: string;
  identityId: string;
  handle: string;
}
