...
export class SqliteAdapter implements StorageAdapter {
   ...
   *
   * Note (post-2026-05-12 identity rewrite): per
   * `feedback_no_backcompat_server_lifecycle`, the `passcode_hash` /
   * `sessions` removal is a one-shot greenfield wipe — operators
   * delete the DB before deploying the post-rewrite server. No
   * forward-migration of legacy passcoded rows is supported.
   */
  ...
  async createIdentity(input: { handle: string }): Promise<Identity> {
    const row: Identity = {
      id: this.idFactory(),
      handle: input.handle,
      createdAt: this.now(),
    ...
    this.db
      .prepare(
        `INSERT INTO identities (id, handle, created_at, is_admin)
         VALUES (?, ?, ?, 0)`
      )
      .run(row.id, row.handle, row.createdAt);
    return row;
  }
  ...
  async deleteIdentityByHandle(handle: string): Promise<void> {
    this.db
      .prepare(`DELETE FROM identities WHERE handle = ?`)
      .run(handle);
  }
