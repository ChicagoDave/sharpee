 ...
 *
 * Method groupings:
 * - Identity ............ ADR-161 (amended 2026-05-12: no passcode).
 *                         `{id, handle}` is the entire credential.
...
export interface StorageAdapter {
  // ── Identity ──────────────────────────────────────────────────
  /** Insert a new identity. Throws on UNIQUE collision on `handle`. */
  createIdentity(input: { handle: string }): Promise<Identity>;
  getIdentityByHandle(handle: string): Promise<Identity | null>;
  getIdentityById(id: string): Promise<Identity | null>;
  /** Hard-delete by handle (ADR-161 erase). Idempotent — no-op if the
   * handle is unknown. Cascades to participants/sessions via FK. */
  deleteIdentityByHandle(handle: string): Promise<void>;
