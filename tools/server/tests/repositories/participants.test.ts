 ...
 *   DOES: inserts participant rows; reconnects existing tokens (setting
 *         connected=1 and updating display_name); mutates tier, muted,
 *         connected; lists participants per room ordered by joined_at;
 *         finds the earliest-joined connected participant (succession
 *         chain).
...
describe('ParticipantsRepository', () => {
  ...
  it('createOrReconnect with a new token creates a participant at tier=participant', () => {
    ...
    const p = participants.createOrReconnect({ room_id, token: 'abc', display_name: 'Alice' });
  ...
  it('createOrReconnect with an existing token marks connected=1 and updates display_name', () => {
    const room_id = setupRoom(db);
    const participants = createParticipantsRepository(db);
    const first = participants.createOrReconnect({ room_id, token: 'abc', display_name: 'Alice' });
    participants.setConnected(first.participant_id, false);

    const reconnected = participants.createOrReconnect({ room_id, token: 'abc', display_name: 'Alicia' });
  ...
  it('listForRoom orders by joined_at ascending', async () => {
    const room_id = setupRoom(db);
    const participants = createParticipantsRepository(db);
    const a = participants.createOrReconnect({ room_id, token: 'a', display_name: 'A' });
    // Small delay so joined_at differs.
    await new Promise((r) => setTimeout(r, 5));
    const b = participants.createOrReconnect({ room_id, token: 'b', display_name: 'B' });
