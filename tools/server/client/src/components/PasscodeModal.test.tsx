 ...
 *   DOES: validates that passcode is non-empty; reads the stored identity
 *         for credentials; calls resolveCodeFn(code) to translate a
 *         passcode to a room_id; if the modal was opened for a specific
 *         expectedRoomId and the resolution lands on a different room,
 *         surfaces an "Invalid passcode." field error without attempting
 *         a join; on a successful resolution, calls joinRoomFn(room_id,
 *         {handle, passcode, captcha_token}); on a successful join,
 *         persists `sharpee.token.<room_id>` and invokes onJoined.
 *   WHEN: the user submits the passcode form.
 *   BECAUSE: the passcode is the room's private gate (ADR-153 Decision 3);
 *            the token is required for reconnect (Decision 5); per
 *            ADR-161 the Handle is the display name and (handle,
 *            passcode) is the auth credential pair.
 *   REJECTS WHEN: client-side: empty/whitespace code; identity missing at
 *                 submit. Server: room_not_found (resolve) → passcode
 *                 field error; wrong-room resolution (list click) →
 *                 passcode field error; unknown_handle / bad_passcode
 *                 (join) → form-level "identity invalid" alert;
...
describe('<PasscodeModal>', () => {
  ...
  it('resolves the code, joins with stored identity credentials, persists the token, invokes onJoined', async () => {
  ...
  it('identity missing at submit → form-level alert, no resolve', async () => {
    window.localStorage.clear();
    const resolveCodeFn = vi.fn();
    const joinRoomFn = vi.fn();
    renderModal({ resolveCodeFn, joinRoomFn });
  ...
  it('join unknown_handle → form-level "identity no longer valid" alert', async () => {
    const resolveCodeFn = vi.fn().mockResolvedValue(OK_RESOLVE);
    const joinRoomFn = vi
      .fn()
      .mockRejectedValue(
  ...
  it('join bad_passcode → form-level "identity no longer valid" alert', async () => {
    const resolveCodeFn = vi.fn().mockResolvedValue(OK_RESOLVE);
    const joinRoomFn = vi
      .fn()
      .mockRejectedValue(
