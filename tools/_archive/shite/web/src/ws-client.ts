...
/** Server's confirmation of a successful `hello`. */
...
export interface WsClientOptions {
  ...
  roomId: string;
  /** Caller's handle. Sent as the first frame `{type:'hello', handle}`
   * per the 2026-05-12 ADR-161 amendment. The server pauses dispatch
   * until this lands; an unknown handle closes the socket 4001. */
  handle: string;
...
export class WsClient {
  ...
  private readonly handleOpen = (): void => {
    ...
    // First frame on every newly-opened socket is `hello` per the
    // 2026-05-12 ADR-161 amendment. The server pauses dispatch until
    // this lands; sending it before any other queued frame is the
    // contract.
    this.socket?.send(
      JSON.stringify({
        type: 'hello',
