...
/**
 * First frame on every newly-opened socket. The server pauses
 * dispatch until it lands; anything else first closes the socket
 * with `4000 hello_required`. Per the 2026-05-12 ADR-161 amendment,
 * the handle IS the credential — no passcode field.
 *
...
/**
 * Server's response to a successful `hello`. Confirms the identity
 * the server bound to the connection so the client can sanity-check
 * its localStorage state (e.g. if the admin erased the handle, the
 * connection would have closed with 4001 before reaching here).
 */
