...
export async function registerWebSocketRoute(
  ...
  app.get(
    '/ws',
    { websocket: true },
    async (socket, _request: FastifyRequest) => {
      // Per the 2026-05-12 ADR-161 amendment, auth is the first inbound
      // frame, not the upgrade URL. We hold the socket in a "pre-hello"
      // state until the client sends `{type:'hello', handle}`. Anything
      // else closes the socket with 4000; an unknown handle closes 4001.
      let client: ClientConnection | null = null;

      socket.on('message', (raw: Buffer) => {
        if (client === null) {
          void handleHello(socket, raw, options.adapter)
            .then((resolved) => {
              if (!resolved) return;
              client = resolved;
            });
...
/**
 * Process the first frame on a freshly-opened socket. Returns the
 * built `ClientConnection` on success, or `null` after closing the
 * socket on failure. The caller binds the returned value into the
 * dispatcher closure so subsequent frames route normally.
 *
 * Close codes (application-level, 4xxx range per RFC 6455):
 *   - `4000 hello_required` — first frame was not `{type:'hello',
 *     handle: string}`.
 *   - `4001 unknown_handle` — handle is not in the identities table
 *     (caller's localStorage is stale; the user must re-claim).
 */
async function handleHello(
  ...
  let parsed: { type?: unknown; handle?: unknown };
  try {
  ...
  if (parsed.type !== 'hello' || typeof parsed.handle !== 'string') {
    const errorFrame: OutboundMessage = {
      type: 'error',
      code: 'invalid_message',
      detail: 'first frame must be {type:"hello", handle:string}',
    };
    socket.send(JSON.stringify(errorFrame));
    socket.close(4000, 'hello_required');
    return null;
  }
  const identity = await adapter.getIdentityByHandle(parsed.handle);
  if (!identity) {
    const errorFrame: OutboundMessage = {
      type: 'error',
      code: 'unknown_handle',
    ...
    socket.send(JSON.stringify(errorFrame));
    socket.close(4001, 'unknown_handle');
    return null;
  }
...
async function handleInbound(
  ...
  switch (message.type) {
    ...
    case 'command:submit':
      ...
      return;
    case 'hello':
      // hello after the connection is already established is a
      // protocol bug — the handshake completed once. Reject without
      // mutating state.
      client.send({
        type: 'error',
        code: 'invalid_message',
