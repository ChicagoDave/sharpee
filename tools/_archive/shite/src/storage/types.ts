...
/**
 * ADR-161 identity row, simplified per 2026-05-12: no passcode.
 *
 * The user picks a unique handle; that handle IS the entire credential.
 * Anyone who claims `alice` on any device IS alice. Accepted threat
 * model for a small IF community. Cross-device "reclaim" is trivial —
 * type your handle again on the new device.
 *
 * `isAdmin` is a sticky bit on the row; bootstrap CLI flips it for
 * known handles. Anyone who claims an admin handle becomes admin —
 * same trust posture as the user model.
 */
